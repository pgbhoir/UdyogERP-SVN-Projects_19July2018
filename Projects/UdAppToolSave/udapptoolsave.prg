...
&&& Added by suraj Kumawat for bug-30880  on date 30-11-2017  start
If Type('_curvouobj.PcvType') = 'C' And Used('MAIN_VW')

	If _curvouobj.EditMode = .T.  Or  _curvouobj.AddMode = .T.

		If Type("MAIN_VW.ecomac_id") = "N" And  Type("LMC_VW.ECOMMAC_NM") = "C"
			If !Empty(LMC_VW.ECOMMAC_NM)
				msqlstr=""
				msqlstr="Select ac_id From ac_mast Where supp_type = 'E-Commerce' and ac_name = ?LMC_VW.ECOMMAC_NM"
				sql_con = _curvouobj.sqlconobj.dataconn([EXE],company.dbname,msqlstr,[tmecomtbl],"_curvouobj.nHandle",_curvouobj.DataSessionId,.F.)
				If sql_con > 0 And Used('tmecomtbl')
					If Reccount('tmecomtbl') > 0
						Replace main_vw.ecomac_id	With tmecomtbl.ac_id In main_vw
					Endif
				Endif
			Else
				Replace main_vw.ecomac_id	With 0 In main_vw
			Endif
		Endif
	Endif
Endif
&&& Added by suraj Kumawat for bug-30880 on date 30-11-2017 end
...
&& Added by Shrikant S. on 14/11/2017 for GST Bug-30857		&& Start
If (Type('_screen.activeform.Behave')='C' And Used('ITEM_VW'))
	BatchMandatory=checkbatchexp()
Endif
...
*!*						If _Tally>0   &&Commented by Priyanka B on 02082017 for Bug-30663
					If _Tally<0  &&Modified by Priyanka B on 02082017 for Bug-30663
						=Messagebox("No Batch Selected ...",0+64,vumess)
						If Used('_tempcur1')
							Use In _tempcur1
						Endif
...
&& Added by Shrikant S. on 16/12/2017 for Auto updater 13.0.5		&& Start
If (Type('_screen.activeform.PCVTYPE')='C' And Used('MAIN_VW'))
	If _Screen.ActiveForm.pcvtype="GB" And _Screen.ActiveForm.EditMode
		If !Used('Debit_vw') Or !Used('GSTPay_vw1') Or !Used('GSTPay_vw2')
			=Messagebox("Please click on done button in GST Details and GST RCM Details.",0,vumess)
			Return .F.
		Endif
	Endif
Endif
&& Added by Shrikant S. on 16/12/2017 for Auto updater 13.0.5		&& End

If Used(_Malias)  &&Added by Priyanka B on 23062018 for Sprint 1.0.1
	If !Empty(_Malias)
		Select &_Malias
	Endif
	If Betw(_mRecNo,1,Reccount())
