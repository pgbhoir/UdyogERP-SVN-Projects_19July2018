...
Procedure mfgexpdtchk		&& Procedure to Check the Manufacturing Date with Expiry Date
	...
	*!*	Endif
	...
	If Inlist(main_vw.entry_ty,'PK','SK')
		If Upper(Alltrim(Strextract(oobject.ucontrolsource,'.'))) = 'EXPDT'
			_curform = _Screen.ActiveForm
			msqlstr = "Select prExpAlertDays from it_mast where It_code=?Item_vw.It_code"
			_curretval = _curform.sqlconobj.dataconn([EXE],company.dbname,msqlstr,"_TmpAlert","This.Parent.nHandle",_curform.DataSessionId,.F.)
			_curform.sqlconobj.sqlconnclose("This.Parent.nHandle")
			If _curretval > 0 And Used('_TmpAlert')
				Select _TmpAlert
				If Reccount()>0
					If _TmpAlert.prExpAlertDays > 0
						cAlert = Ttod(main_vw.Date)+_TmpAlert.prExpAlertDays
						If cAlert >= item_vw.expdt And !Empty(item_vw.expdt)
							Messagebox("Selected batch is expired/near to expire within "+Alltrim(Str(_TmpAlert.prExpAlertDays))+" days!!",0+64,vumess)
						Endif
						Use In _TmpAlert
		...
		If (_txtObj.Parent.Parent.Parent.oParForm.IsBarCode =.T.)
			If (Alltrim(_txtObj.Tag) <> Alltrim(_valCur))
				...
				Else
					Locate
				Endif
				If !Empty(Alltrim(BarCodeMast_vw.Fld_Nm2))  &&Added by Priyanka B on 25102018 for Bug-31963(AU 1.0.2)
					...
					If !Empty(lcfld_nm)
						...
						Endif
					Endif
					&&Added by Priyanka B on 25102018 for Bug-31963(AU 1.0.2) Start
				Else
					_txtObj.cErrMsg="'Barcode field not defined in Barcode Master!!'"
					_txtObj.Parent.SetFocus()
					Return .F.
				Endif
				&&Added by Priyanka B on 25102018 for Bug-31963(AU 1.0.2) End
			Endif
		Endif
...
Procedure SerTaxRecApply
	...
	*!*		If !Empty(coadditional.ConstBusi) And !Empty(lcserty)		&& Added by Shrikant S. on 20/10/2016 for GST		&& Commented by Shrikant S. on 28/03/2019 for Bug-32266
	If  !Empty(lcserty)		&& Added by Shrikant S. on 28/03/2019 for Bug-32266
		...
		*!*		etsql_str = "Select Top 1 RevSerProvider,RevSerReceiver From Sertax_mast Where Name = ?AcdetAlloc_vw.Serty"
		...
		If !Empty(_mAcSerDed_Type)
			...
			Endif
		Endif
		*MESSAGEBOX("ded_type="+_mAcSerDed_Type +"=_mSerTaxRecApply ="+TRANSFORM(_mSerTaxRecApply)+"=_SerTaxTranType ="+_SerTaxTranType +"=lcserty="+lcserty)
	Endif
	If _oldconval <= 0
...
Procedure AdvtypeSetting
	Parameters _Object  &&Added by Priyanka B on 27032019 for Bug-32067
	...
	*!*	If(main_vw.tdspaytype=2 And curObj.addmode)
	If(curObj.addmode) And llvalid
		...
		&&Added by Priyanka B on 27032019 for Bug-32067 Start
		If Vartype(oGlblPrdFeat)='O'
			If oGlblPrdFeat.UdChkProd('vuisd') Or oGlblPrdFeat.UdChkProd('isdkgen')
				_Object.defahelp="PURCHASES,ISD INVOICE PASSING"
			Endif
		Endif
		&&Added by Priyanka B on 27032019 for Bug-32067 End
	Else
		Return .F.
	Endif
	Return
...
Procedure debitaceffect_bp()
	...
	*!*	_vservtype = findallocservicetype()			&& Commented by Shrikant S. on 09/02/2017 for GST
	...
	&& Added by Shrikant S. on 01/11/2019 for Bug-32836		&& Start
	If _llImport=.T.
		_spacct=Iif(CheckIsUnderSEZ() And isservitem(),"",_spacct)
	Endif
	&& Added by Shrikant S. on 01/11/2019 for Bug-32836		&& End
	...
	*!*	Endif
	...
	Do Case
		...
		Case Inlist(Alltrim(main_vw.againstgs),"PURCHASES","SERVICE PURCHASE BILL")
			llImport=IsUnderImport()
			&&Commented by Priyanka B on 24082018 for Accounting Effect Changes Start
			...
			*!*				Endif
			...
			&&Commented by Priyanka B on 24082018 for Accounting Effect Changes End

			&&Modified by Priyanka B on 24082018 for Accounting Effect Changes Start
			Do Case
				Case _fldnm="CGST_AMT"
				...
				Case _fldnm="COMPCESS"
					_dacname="CCess Payable"
			Endcase
			&&Modified by Priyanka B on 24082018 for Accounting Effect Changes End
			...
			*!*		Endcase
			&& Commented by Shrikant S. on 10/06/2017 for GST		&& End
			&&Added by Priyanka B on 10042019 for Bug-32067 Start
		Case Inlist(Alltrim(main_vw.againstgs),"ISD INVOICE PASSING")
			Do Case
				Case _fldnm="CGST_AMT"
					_dacname="CGST Payable"
				Case _fldnm="SGST_AMT"
				...
				Case _fldnm="COMPCESS"
					_dacname="CCess Payable"
			Endcase
			&&Added by Priyanka B on 10042019 for Bug-32067 End
		Otherwise
...
Procedure DebitPosting_GC
	...
	*!*	Endif
	...
	Do Case
		...
		Case Inlist(Alltrim(main_vw.againstgs),"PURCHASES","SERVICE PURCHASE BILL")
			*!*				llImport= IsUnderImport()				&& Commented by Shrikant S. on 14/09/2019 for Bug-32836
			*!*		llImport=Iif(!CheckIsUnderSEZ(),IsUnderImport(),.T.)					&& Added by Shrikant S. on 14/09/2019 for Bug-32836
			llImport= IsUnderImport()				&& UnCommented by Shrikant S. on 31/10/2019 for Bug-32836
	...
	Do Case
		...
		Case Inlist(Alltrim(main_vw.againstgs),"PURCHASES","SERVICE PURCHASE BILL")
			...
			*!*		llImport=Iif(!CheckIsUnderSEZ(),IsUnderImport(),.T.)					&& Added by Shrikant S. on 14/09/2019 for Bug-32836
			...
			&& Added by Shrikant S. on 01/11/2019 for Bug-32836		&& Start
			If llImport =.T.
				llImport =Iif(CheckIsUnderSEZ() And isservitem(),.F.,.T.)
			Endif
			&& Added by Shrikant S. on 01/11/2019 for Bug-32836		&& End
			If llImport
				Do Case
...
Procedure IsUnderImport
	...
	llImport= Iif(Alltrim(tmp_type.st_type)="OUT OF COUNTRY" Or Inlist(Alltrim(tmp_type.supp_type),"SEZ","Import","EOU")  ,.T.,.F.)		&& unCommented by Shrikant S. on 02/11/2019 for Bug-32836
	*!*		llImport= Iif(Alltrim(tmp_type.st_type)="OUT OF COUNTRY" Or Inlist(Alltrim(tmp_type.supp_type),"Import","EOU")  ,.T.,.F.)		&& Added by Shrikant S. on 14/09/2019 for Bug-32836		&& Commented by Shrikant S. on 02/11/2019 for Bug-32836
	*!*	llImport= Iif(Alltrim(tmp_type.st_type)="OUT OF COUNTRY" Or Inlist(Alltrim(tmp_type.supp_type),"SEZ","Import","EOU")  ,.T.,.F.)		&& Added by Shrikant S. on 02/11/2017 for GST Bug-30830		&& Commented by Shrikant S. on 14/09/2019 for Bug-32836
...
Procedure calc_totalgst
	*!*	Replace gro_amt With main_vw.cgst_amt+main_vw.sgst_amt+main_vw.igst_amt+main_vw.compcess,net_amt With main_vw.cgst_amt+main_vw.sgst_amt+main_vw.igst_amt+main_vw.compcess  In main_vw  &&Commented by Priyanka B on 23032019 for Bug-32067
	&&Modified by Priyanka B on 23032019 for Bug-32067 Start
	Replace gro_amt With main_vw.cgst_amt+main_vw.sgst_amt+main_vw.igst_amt+main_vw.compcess+main_vw.cgst_oamt+main_vw.cgst_famt+main_vw.sgst_oamt+main_vw.sgst_famt,;
		net_amt With main_vw.cgst_amt+main_vw.sgst_amt+main_vw.igst_amt+main_vw.compcess+main_vw.cgst_oamt+main_vw.cgst_famt+main_vw.sgst_oamt+main_vw.sgst_famt In main_vw
	&&Modified by Priyanka B on 23032019 for Bug-32067 End
			...
			If _curretval > 0 And Used('_Tmpval')
				...
				If !Empty(lcEntrytbl)
					...
					If _curretval >0
						...
						Select main_vw
						Gather Name oMain_vw Memo
						Replace paymentno With oobject.Value,Cheq_no With "",tdspaytype With 2 ,entry_ty With _curform.pcvtype In main_vw		&& Changed by Shrikant S. on 25/03/2019 for Bug-30939

						msqlstr="Select * From "+Alltrim(lcEntrytbl)+"Item Where Tran_cd=?lcTrancd and Entry_ty=?lcEntry"
						_curretval = _curform.sqlconobj.dataconn([EXE],company.dbname,msqlstr,"_Tmpitem","_curform.nHandle",_curform.DataSessionId,.F.)

						_curform.rbtntdspaytype.Value=2					&& Added by Shrikant S. on 18/06/2019 for Bug-30939
						If _curretval >0
							Select _Tmpitem
							Scan
								...
								Gather Name oitem_vw Memo
								Replace entry_ty With main_vw.entry_ty,tran_cd With main_vw.tran_cd,Date With main_vw.Date,Doc_no With main_vw.Doc_no,sr_sr With 'TTTTT' In item_vw		&& Changed by Shrikant S. on 25/03/2019 for Bug-30939
							...
							*!*								Delete From item_vw Where itserial In (Select itserial From _Tmppay Where (cgst_amt+sgst_amt+igst_amt+compcess)=(cgst_adj+sgst_adj+igst_adj+ccess_adj))		&& Commented by Shrikant S. on 28/08/2018 for Installer 2.0.0
							Delete From item_vw Where itserial In (Select itserial From _Tmppay Where (cgst_amt+sgst_amt+igst_amt+compcess)=(cgst_adj+sgst_adj+igst_adj+ccess_adj))	And (cgst_amt+sgst_amt+igst_amt+compcess)>0		&& Added by Shrikant S. on 28/08/2018 for Installer 2.0.0

							lnsumnet=0
							&& Added by Shrikant S. on 18/06/2019 for Bug-30939		&& Start
							lncgstamt=0
							lnsgstamt=0
							lnigstamt=0
							&& Added by Shrikant S. on 18/06/2019 for Bug-30939		&& End
							Select item_vw
							Scan
								Select _Tmppay
								Locate For itserial=item_vw.itserial
								If Found()
									&& Added by Shrikant S. on 25/03/2019 for Bug-30939		&& Start
									lncgst_adj=_Tmppay.cgst_adj
									lnsgst_adj=_Tmppay.sgst_adj
									lnigst_adj=_Tmppay.igst_adj
									lnccess_adj=_Tmppay.ccess_adj
									lnTaxable=_Tmppay.Taxa_adj

									Select item_vw
									Replace u_asseamt With u_asseamt-lnTaxable,cgst_amt With cgst_amt-lncgst_adj,sgst_amt With sgst_amt-lnsgst_adj,igst_amt With igst_amt-lnigst_adj,compcess With compcess-lnccess_adj In item_vw

									Replace rate With u_asseamt,qty With 1,staxable With u_asseamt,gstrate With _Tmppay.CGST_PER+_Tmppay.SGST_PER+_Tmppay.IGST_PER In item_vw
									Replace gro_amt With u_asseamt+cgst_amt+ sgst_amt+ igst_amt+compcess,AMTINCGST With 0,amtexcgst With 0 In item_vw

									lnsumnet=lnsumnet+item_vw.gro_amt
									&& Added by Shrikant S. on 18/06/2019 for Bug-30939		&& Start
									lncgstamt=lncgstamt+item_vw.cgst_amt
									lnsgstamt=lnsgstamt+item_vw.sgst_amt
									lnigstamt=lnigstamt+item_vw.igst_amt
									&& Added by Shrikant S. on 18/06/2019 for Bug-30939		&& End

									&& Added by Shrikant S. on 25/03/2019 for Bug-30939		&& End
									*								_curform.itemgrdbefcalc(_curform.Voupage.Page1.Grditem.columncount)
								Endif
								Select item_vw
							Endscan
							Replace cgst_amt With lncgstamt,sgst_amt With lnsgstamt,igst_amt With lnigstamt In main_vw		&& Added by Shrikant S. on 18/06/2019 for Bug-30939
							Replace net_amt With lnsumnet,gro_amt With lnsumnet In main_vw					&& Added by Shrikant S. on 25/03/2019 for Bug-30939
							_curform.voupage.Page1.txtItemTotal.Value=lnsumnet 								&& Added by Shrikant S. on 25/03/2019 for Bug-30939
							_curform.itempage=.T.
							...
							*!*							Endif
							*						_curform.SacId=main_vw.sac_id
							*!*							_curform.itemgrdbefcalc(_curform.Voupage.Page1.Grditem.columncount-1)
							_curform.accountsposting()									&& Added by Shrikant S. on 25/03/2019 for Bug-30939
							_curform.Refresh()
						Endif
					Endif
...
Procedure act_deactcontrol_PT()
	...
	*!*	If IsUnderImport() And !llIsthirdparty   &&Modified by Priyanka B on 16042018 for AU 13.0.6			&& Commented by Shrikant S. on 14/09/2019 for Bug-32836
	*!*		If (IsUnderImport() Or CheckIsUnderSEZ()) And !llIsthirdparty   && Added by Shrikant S. on 14/09/2019 for Bug-32836		&& Commented by Shrikant S. on 02/11/2019 for Bug-32836
	If IsUnderImport() And !llIsthirdparty  		&& Added by Shrikant S. on 02/11/2019 for Bug-32836
		itemcount=0
		Select item_vw
		lnrecno=Iif(!Eof(),Recno(),0)
	...
	Else
		If llIsthirdparty
			...
			*!*			If IsUnderImport()   &&Modified by Priyanka B on 16042018 for AU 13.0.6			&& Commented by Shrikant S. on 14/09/2019 for Bug-32836
			*!*				If IsUnderImport()  Or CheckIsUnderSEZ()  && Added by Shrikant S. on 14/09/2019 for Bug-32836		&& Commented by Shrikant S. on 02/11/2019 for Bug-32836
			If IsUnderImport()   		&& Added by Shrikant S. on 02/11/2019 for Bug-32836
				itemcount=0
				Select item_vw
				lnrecno=Iif(!Eof(),Recno(),0)
...
Procedure GetSrno
	...
	*!*	Endif
	...
	Endscan
	*!*		If Alltrim(lcsrno)<>""			&& Added by Shrikant S. on 29/09/2018 for Installer ENT 2.0.0		&& Commneted by Shrikant S. on 21/08/2019 for AU 2.2.1
	If !Empty(lcsrno)			&& Added by Shrikant S. on 21/08/2019 for AU 2.2.1
	...
	Endif							&& Added by Shrikant S. on 29/09/2018 for Installer ENT 2.0.0
	frmxtra.Refresh()
...
Procedure debitaceffect_pt_reversal()
	...
	&& Added by Shrikant S. on 13/11/2018 for Auto updater 2.0.1		&& Start
	If Type('_mseravail') = 'C'
		If Inlist(_mseravail,"SERVICES")
			_isadv = .T.
		Endi
	Endif
	If _isadv =.T.
	...
	Else
		...
		&& Added by Shrikant S. on 13/11/2018 for Auto updater 2.0.1		&& End
		Do Case
			Case _fldnm="CGSRT_AMT"
				_vacnm="Input CGST"
			Case _fldnm="SGSRT_AMT"
	...
	Endif && Added by Shrikant S. on 13/11/2018 for Auto updater 2.0.1
...
Procedure GetTransDetails
	...
	If(Gettransdet_cur.gstin<>'UNREGISTERED')
		Replace TRANS_ID With Gettransdet_cur.gstin In (MTRANS_ID)
	Else
		Replace TRANS_ID With '' In (MTRANS_ID)
	Endif
...
***** Added by SAchin N. S. on 21/09/2018 for Bug-31756 -- Start
Procedure GetSrlInvType
	Lparameters _curObjct,_oFrmObj

	_curFrm = _Screen.ActiveForm
	msqlstr="Select top 1 iTran_cd from it_srstk where it_code=?It_mast_vw.It_code"
	_oFrmObj.sqlconobj.dataconn("EXE",company.dbname,msqlstr,"_curSrInvLst","This.Parent.nhandle",_oFrmObj.DataSessionId)
	Select _curSrInvLst
	If Reccount()>0
		Messagebox("Cannot change Inventory Type as Transaction have been done against the Supply.",0+64,vumess)
		Return .F.
	Endif
...
Endproc
***** Added by SAchin N. S. on 21/09/2018 for Bug-31756 -- End



&& Added by Shrikant S. on 10/11/2018 for Auto updater 2.0.1		&& Start
Procedure CheckBatchNoforHold
	Lparameters _txtObj,_valCur
	...
	*!*		Return .F.
	*!*	Endif
	msqlstr="select top 1 batchno,mfgdt,expdt,BatchonHold,inc_gst from pRetBatchDetails  where batchno= ?_valCur1 and It_code="+Alltrim(Str(item_vw.it_code))  &&Added by Priyanka B on 11122018 for Bug-31930
	*!*	_curretval = _curform.sqlconobj.dataconn([EXE],company.dbname,"select top 1 batchno,mfgdt,expdt,BatchonHold,inc_gst from pRetBatchDetails  where batchno= ?_valCur1 and It_code=?Item_vw.It_code","_TmpAlloc","This.Parent.nHandle",_curform.DataSessionId,.F.)  &&Commented by Priyanka B on 11122018 for Bug-31930
	_curretval = _curform.sqlconobj.dataconn([EXE],company.dbname,msqlstr,"_TmpAlloc","This.Parent.nHandle",_curform.DataSessionId,.F.)  &&Modified by Priyanka B on 11122018 for Bug-31930
	_curform.sqlconobj.sqlconnclose("This.Parent.nHandle")
	If _curretval > 0 And Used('_TmpAlloc')
		Select _tmpalloc
		If Reccount()>0
			If _tmpalloc.expdt < main_vw.Date
				_txtObj.cErrMsg="'Batch is expired. Cannot continue...!!'"
				_txtObj.Parent.SetFocus()
				Return .F.
			Endif
			If _tmpalloc.BatchonHold =.T.
				_txtObj.cErrMsg="'Batch is on hold. Cannot continue...!!'"
				_txtObj.Parent.SetFocus()
				Return .F.
			Endif
			If Type('Item_vw.GSTIncl')='L'
				Replace GSTIncl With _tmpalloc.inc_gst,mfgdt With _tmpalloc.mfgdt,expdt With _tmpalloc.expdt In item_vw
			Endif
		Endif
	Endif
	Return .T.
Endproc
&& Added by Shrikant S. on 10/11/2018 for Auto updater 2.0.1		&& End


&&Added by Priyanka B on 04122018 for Bug-31930 Start
Procedure EnableDisable_PU
	curObj=_Screen.ActiveForm
	*!*	If main_vw.entry_ty='PU'  &&Commented by Priyanka B on 05012019 for Bug-32119
	If Inlist(main_vw.entry_ty,'PU','RA')  &&Modified by Priyanka B on 05012019 for Bug-32119
		If Type('main_vw.ISEXPDSTK')<>'U'
			If main_vw.ISEXPDSTK = .F.
				Replace main_vw.STKEXPGDAY With 0 In main_vw
				_Screen.ActiveForm.Refresh
			Endif
		Endif
	Endif
...
&& Added by Shrikant S. on 11/12/2018 for Installer 2.1.0		&& Start
Procedure update_expdt
	Parameters oexpdt

	If !Empty(item_vw.mfgdt) And Empty(item_vw.expdt)
		oexpdt.Value=Gomonth(item_vw.mfgdt,12)
		Replace expdt With Gomonth(item_vw.mfgdt,12) In item_vw
	Endif
	Return
	&& Added by Shrikant S. on 11/12/2018 for Installer 2.1.0		&& End


	&& Added by Shrikant S. on 14/12/2018 for Installer 2.1.0		&& Start
Procedure SetlooseQty
	Parameters _Object

	_Object.Tag=Transform(_Object.Value)

Endproc


Procedure Update_LooseQtyToQty
	Parameters _Object

	If Val(_Object.Tag)# _Object.Value
		Replace qty With qty-Val(_Object.Tag)+_Object.Value In item_vw
	Endif
...
&&Added By Rupesh G. on 28/01/2019 for 32194--Start
Procedure job_detail()
	Local zretval
	zretval=''
	sqlconobj=Newobject('sqlconnudobj',"sqlconnection",xapps)
	nhandle=0

	If  main_vw.entry_ty='PI'
		sq1="select U_ENGINNO "
		sq1=sq1+", U_KEYNO "
		sq1=sq1+", U_BATTERYN "
		sq1=sq1+", U_WRCNO "
		sq1=sq1+", U_MODEL "
		...
		sq1=sq1+", party_nm "
		sq1=sq1+", u_vehiclen "
		sq1=sq1+" from somain  where somain.U_JOBNO=?main_vw.U_JOBNO"

		nretval = sqlconobj.dataconn([EXE],company.dbname,sq1,"zPM_vw","nHandle",_Screen.ActiveForm.DataSessionId)
		If nretval<0
			Return .F.
		Endif
		Replace U_CHASSISN With  zPM_vw.U_CHASSISN In main_vw
		Replace u_vehiclen With  zPM_vw.u_vehiclen In main_vw
		Replace party_nm With  zPM_vw.party_nm In main_vw
		Replace U_ENGINNO With  zPM_vw.U_ENGINNO In main_vw
		Replace U_KEYNO With  zPM_vw.U_KEYNO In main_vw
		Replace U_BATTERYN With  zPM_vw.U_BATTERYN In main_vw
		Replace U_WRCNO With  zPM_vw.U_WRCNO In main_vw
		Replace U_model With  zPM_vw.U_model In main_vw

	Endif


	If main_vw.entry_ty='IP'
	...
	If MVU_USER_ROLES='WORKSHOP' And main_vw.entry_ty='DC'
	...
	If MVU_USER_ROLES='WORKSHOP' And main_vw.entry_ty='ST'
		Replace inv_sr With  'WORKSHOW1819' In main_vw
	Endif


	If MVU_USER_ROLES='WORKSHOP' And main_vw.entry_ty='ST' And main_vw.U_STYPE='Job Card'
...
Procedure check_enginno()
	Local zretval
	zretval=''
	sqlconobj=Newobject('sqlconnudobj',"sqlconnection",xapps)
	nhandle=0

	If MVU_USER_ROLES='WORKSHOP' And main_vw.entry_ty='SO'

		sq1="select It_SrStk.U_ENGINNO "
		sq1=sq1+", It_SrStk.U_KEYNO "
		sq1=sq1+", It_SrStk.U_BATTERYN"
		sq1=sq1+", It_SrStk.U_WRCNO "
		sq1=sq1+", IT_MAST.it_name "
		sq1=sq1+"from It_SrStk inner join IT_MAST on (It_SrStk.It_Code=IT_MAST.It_code) where It_SrStk.SERIALNO=?main_vw.U_CHASSISN"
		...
		Replace U_BATTERYN With  zPM_vw.U_BATTERYN In main_vw
		Replace U_WRCNO With  zPM_vw.U_WRCNO In main_vw
		Replace U_model With  zPM_vw.it_name In main_vw

	Endif

Procedure calDueDate
	Set Step On
	mdue_dton=Iif(Type('main_vw.VUDays')<>'U','main_vw.vudays','lmc_vw.vudays')
	mvw=Iif(Type('main_vw.VUDate')<>'U','main_vw','lmc_vw')

	Sele main_vw
	Do Case
		Case !Empty(&mdue_dton) And &mdue_dton > 0
			Replace VUDate With (Ttod(main_vw.Date) + (&mdue_dton)) In &mvw
		Otherwise
			Replace VUDate With main_vw.Date In &mvw
	Endcase
Endproc


Procedure chkDuplEngNo_1
	Lparameters _txtObj,_valCur,_CurRecond
...
Procedure get_vehno
	...
	If Empty(main_vw.AMVehNo)
		Return
	Endif

	If Upper(_curform.mainalias)!='MAIN_VW'
		Return
	Endif
	...
	If Type('_curform.sqlconobj') = 'O'
		...
		If Upper(_curform.mainalias)='MAIN_VW'
			...
			sq1="Select entry_ty,bcode_nm,ext_vou,mEntry=case when Bcode_nm<>'' then Bcode_nm else (case when ext_vou=1 then '' else entry_ty end) end From Lcode where entry_ty=?Main_vw.entry_ty"
			nretval = _curform.sqlconobj.dataconn([EXE],company.dbname,sq1,"_entryty","_curform.nHandle",_etdatasessionid)
			Select _entryty

			sq1="select distinct AMVehNo,AMModel,AMChasNo,party_nm from "+Alltrim(_entryty.mEntry)+"main where AMVehNo= ?main_vw.AMVehNo "


			nretval = _curform.sqlconobj.dataconn([EXE],company.dbname,sq1,"_AMVehNo","_curform.nHandle",_etdatasessionid)

			If nretval > 0 And Used('_AMVehNo') And Reccount('_AMVehNo')>0

				Replace AMVehNo With _AMVehNo.AMVehNo,AMModel With _AMVehNo.AMModel ,AMChasNo With _AMVehNo.AMChasNo,party_nm With _AMVehNo.party_nm  In main_vw
			Else
				If Empty(_AMVehNo.AMVehNo)
					Replace AMModel With '' ,AMChasNo With '' In main_vw
				Endif
...
Procedure get_vehno1
	...
	If Type('_curform.sqlconobj') = 'O'
		...
		If Upper(_curform.mainalias)='MAIN_VW'
			...
			sq1="select distinct AMVehNo,stmainadd.AMModel,SERIALNO as 'AMChasNo',party_nm from stmain inner join STMAINadd ON (stmain.Tran_cd=stmainadd.Tran_cd) where AMVehNo= ?main_vw.AMVehNo"

			nretval = _curform.sqlconobj.dataconn([EXE],company.dbname,sq1,"_AMVehNo","_curform.nHandle",_curform.DataSessionId)

			If Used('_AMVehNo')
				If nretval > 0 And Reccount('_AMVehNo')>0

					Replace AMVehNo With _AMVehNo.AMVehNo,party_nm With _AMVehNo.party_nm  In main_vw
					Replace AMModel With _AMVehNo.AMModel,SERIALNO With _AMVehNo.AMChasNo In mainadd_vw
				Else
					If Empty(_AMVehNo.AMVehNo)

						Replace AMModel With '',SERIALNO With ''  In mainadd_vw
					Endif
				Endif
				If Used('_AMVehNo')
					Use In _AMVehNo
	...
	Return .T.
Endproc
&&Added By Rupesh G. on 28/01/2019 for 32194--End

&& Added by Shrikant S. on 24/08/2019 for AU 2.2.1		&& Start
Procedure GetAsseAmount
	retvalue=(item_vw.qty * item_vw.rate)
	If _Screen.ActiveForm.Multi_Cur = .T.
		If Upper(Alltrim(main1_vw.Fcname)) != Upper(Alltrim(company.Currency)) And !Empty(main1_vw.Fcname)
			retvalue=UDAppvouItemwiseAssess() * main_vw.fcexrate
		Endif
	Endif
	Return retvalue
	&& Added by Shrikant S. on 24/08/2019 for AU 2.2.1		&& End

	&&Added by Priyanka B on 25072019 for Bug-32747 Start
Procedure fCalQtyCutPer
	Lparameters oObj
	_curform = _Screen.ActiveForm
	_etdatasessionid = _curform.DataSessionId
	If [efabric] $ vchkprod
		If Type('Item_vw.fCutPer')<>'U' And Type('Item_vw.fPcs')<>'U'
			If item_vw.fCutPer > 0
				If Inlist(_curform.pcvtype,"RD","W1","OC","W2")

					&&Commented by Priyanka B on 30112019 for Bug-33074 Start
					*!*						If Inlist(_curform.pcvtype,"RD","OC") And Empty(item_vw.flotno)
					*!*							Return .T.
					*!*						Endif
					&&Commented by Priyanka B on 30112019 for Bug-33074 End

					&&Modified by Priyanka B on 30112019 for Bug-33074 Start
					If Inlist(_curform.pcvtype,"RD","OC")
						If Type('item_vw.flotno')='U'
							Return .T.
						Endif
						...
						Endif
					Endif
					&&Modified by Priyanka B on 30112019 for Bug-33074 End

					&&Commented by Priyanka B on 30112019 for Bug-33074 Start
					*!*							If Inlist(_curform.pcvtype,"W1","W2") And Empty(main_vw.flotno)
					*!*								Return .T.
					*!*							Endif
					&&Commented by Priyanka B on 30112019 for Bug-33074 End

					&&Modified by Priyanka B on 30112019 for Bug-33074 Start
					If Inlist(_curform.pcvtype,"W1","W2")
						If Type('main_vw.flotno')='U'
							Return .T.
						Endif
						If Empty(main_vw.flotno)
							Return .T.
						Endif
					Endif
					&&Modified by Priyanka B on 30112019 for Bug-33074 End

					nCalQty = item_vw.fpcs * item_vw.fCutPer
					...
					Do Case
						Case Inlist(_curform.pcvtype,"RD")
							sq1 = "Execute USP_GetLot_Details ?main_vw.entry_ty,?main_vw.party_nm,?item_vw.flotno,'',0.00,?main_vw.tran_cd,?item_vw.itserial"
						Case Inlist(_curform.pcvtype,"W1")
							sq1 = "Execute USP_GetLot_Details ?main_vw.entry_ty,?main_vw.party_nm,?main_vw.flotno,'',0.00,?main_vw.tran_cd,''"
						Case _curform.pcvtype="OC"
							sq1 = "Execute USP_GetLot_Details ?main_vw.entry_ty,'Use For Production',?item_vw.flotno,?item_vw.item,0.00,?main_vw.tran_cd,?item_vw.itserial"
						Case _curform.pcvtype="W2"
							sq1 = "Execute USP_GetLot_Details ?main_vw.entry_ty,'Use For Production',?main_vw.flotno,?item_vw.item,0.00,?main_vw.tran_cd,''"
					Endcase
					nretval = _curform.sqlconobj.dataconn([EXE],company.dbname,sq1,"curlot","_curform.nHandle",_curform.DataSessionId)
					If nretval <=0
						Return .F.
					Endif

					Select flotno,Iif(Isnull(Sum(item_vw.fpcs * item_vw.fCutPer)),0,Sum(item_vw.fpcs * item_vw.fCutPer)) As CutPerQty From item_vw With (Buffering=.T.) Where flotno=sLotno And itserial<>sItserial Group By flotno ;
					...
					If Used("Itref_vw")
						...
						If Reccount('curItref') > 0
							Update a Set a.balqty = (a.baljoqty - Iif(Isnull(b.CutPerQty),0,b.CutPerQty));
								From curlot a Left Outer Join curCalQty b On (a.flotno==b.flotno)
						Else
							Update a Set a.balqty =(a.balwojoqty - Iif(Isnull(b.CutPerQty),0,b.CutPerQty));
								From curlot a Left Outer Join curCalQty b On (a.flotno==b.flotno)
						Endif
					Else
						Update a Set a.balqty =(a.balwojoqty - Iif(Isnull(b.CutPerQty),0,b.CutPerQty));
					...
					If nCalQty > curlot.balqty
						...
						Messagebox("For Lot No. "+Alltrim(item_vw.flotno)+","+Chr(13)+" (No. of Pieces * Length Per Piece) Qty " + Alltrim(Str(nCalQty,16,2)) ;  &&Modified by Prajakta B. on 08-01-2020 for Bug 33074
						+ "/- Cannot Be Greater Than Selected Lot No. Balance Qty " + Alltrim(Str(curlot.balqty,16,2))+"/-",0+16,vumess)
						oObj.Value = Val(oObj.Tag)
						...
						Return .F.
					Else
						Replace item_vw.qty With item_vw.fpcs * item_vw.fCutPer In item_vw
					Endif
				Else
					*!*						Replace item_vw.qty With item_vw.fpcs * item_vw.fCutPer In item_vw  &&Commented by Priyanka B on 25012020 for Bug-33252

					&&Modified by Priyanka B on 25012020 for Bug-33252 Start
					If Used("Itref_vw")
						Select itref_vw
						Select item_vw
						mentry_ty = Alltrim(item_vw.entry_ty)
						...
						If Reccount('curItref') > 0
							...
							If lcode_vw.pickcons

								sFlds="fpcs,fcutper"
								sSumonfld=Alltrim(lcode_vw.sumonfld)

								cnt1=Alines(arr1,sFlds,",")
								cnt2 = Alines(arr2, sSumonfld, ",")
								sDtldata=""
								...
								For i=1 To cnt1
									...
									For j=1 To cnt2
										If Alltrim(Upper(arr1(i))) == Alltrim(Upper(arr2(j)))
											ll=.T.
											Exit
										Endif
									Endfor
					...
					Else
						Replace item_vw.qty With item_vw.fpcs * item_vw.fCutPer In item_vw
					Endif
					&&Modified by Priyanka B on 25012020 for Bug-33252 End
				Endif
				_curform.itemgrdbefcalc(1)  &&Added by Priyanka B on 27092019 for AU 2.2.2
			Endif
		Endif
	Endif
Endproc

Procedure fCalQtyCutPer_whn
	Lparameters oObj
	oObj.Tag = Transform(oObj.Value)
Endproc

Procedure fCalAmtCutPer
	&&Commented by Priyanka B on 13022020 for Bug-33252 Start
	*!*		_curform = _Screen.ActiveForm
	*!*		_etdatasessionid = _curform.DataSessionId
	*!*		If [efabric] $ vchkprod
	*!*			If item_vw.fRatePer = "MTR"
	*!*				Replace item_vw.gro_amt With item_vw.qty * item_vw.rate;
	*!*					,item_vw.u_asseamt With item_vw.qty * item_vw.rate In item_vw
	*!*			Else
	*!*				Replace item_vw.gro_amt With item_vw.fpcs * item_vw.rate;
	*!*					,item_vw.u_asseamt With item_vw.fpcs * item_vw.rate In item_vw
	*!*			Endif
	*!*					_curform.itemgrdbefcalc(1)  &&Added by Priyanka B on 27092019 for AU 2.2.2
	*!*		Endif
	&&Commented by Priyanka B on 13022020 for Bug-33252 End
Endproc

Procedure fPHIdUpdate
	If [efabric] $ vchkprod
		If !Empty(Alltrim(main_vw.fPH_Name))
			_curform = _Screen.ActiveForm
			_etdatasessionid = _curform.DataSessionId
			_curform.nhandle=0
			sq1="SELECT AC_ID FROM AC_MAST WHERE AC_NAME='"+main_vw.fPH_Name+"' "
			nretval = _curform.sqlconobj.dataconn([EXE],company.dbname,sq1,"_updtphid","_curform.nHandle",_curform.DataSessionId)
			If Used("_updtphid")
				If nretval > 0 And Reccount('_updtphid') > 0
					Select main_vw
					Replace fPHAcid With _updtphid.ac_id In main_vw
				Else
					If Used('_updtphid')
						Use In _updtphid
					Endif
...
Procedure fUpdtLotNo_whn
	Lparameters oObj
	oObj.Tag = Transform(oObj.Value)
Endproc

Procedure fUpdtLotNo_valid
	Lparameters oObj
	_curform = _Screen.ActiveForm
	_etdatasessionid = _curform.DataSessionId
	_curform.nhandle=0
	...
	If [efabric] $ vchkprod
		If _curform.addmode Or _curform.editmode
			*!*				If Inlist(_curform.pcvtype,"RD","OC")  &&Commented by Priyanka B on 14022020 for Bug-33252
			If Inlist(_curform.pcvtype,"RD","OC","R2","R3","O6")  &&Modified by Priyanka B on 14022020 for Bug-33252
				*!*				MESSAGEBOX("udappetvalid 1")
				&&Added by Priyanka B on 11102019 for Bug-32747 Start
				If Empty(Alltrim(item_vw.flotno))
					&&Added by Priyanka B on 13112019 for Bug-32747 Start
					If _curform.FlagCancel=.T. Or _curform.curonmouse = .T. Or _curform.DeleRec = .T.
						Return .T.
					Endif
					&&Added by Priyanka B on 13112019 for Bug-32747 End
					Messagebox("Lot No. cannot be empty!!",0+16,vumess)
					Return .F.
				Endif
				&&Added by Priyanka B on 11102019 for Bug-32747 End
				*!*								MESSAGEBOX("udappetvalid 2")
				If !Empty(Alltrim(item_vw.flotno))  			&&Added by Priyanka B on 11102019 for Bug-32747
					*!*								MESSAGEBOX("udappetvalid 3")
					Select item_vw
					Store Recno() To mrecno
					mlotno=item_vw.flotno
					mitserial=item_vw.itserial

					Select flotno,Sum(qty) As aqty From item_vw Where item_vw.itserial<>mitserial And item_vw.flotno=mlotno Group By flotno Into Cursor curQty
					mcurQty = Iif(Isnull(curQty.aqty),0,curQty.aqty)

					Do Case
							*!*							Case Inlist(_curform.pcvtype,"RD")  &&Commented by Priyanka B on 14022020 for Bug-33252
						Case Inlist(_curform.pcvtype,"RD","R2","R3","O6")  &&Modified by Priyanka B on 14022020 for Bug-33252
							*!*										MESSAGEBOX("udappetvalid 4")
							sq1 = "Execute USP_GetLot_Details ?main_vw.entry_ty,?main_vw.party_nm,?item_vw.flotno,'',0.00,?main_vw.tran_cd,?item_vw.itserial"
						Case _curform.pcvtype="OC"
							sq1 = "Execute USP_GetLot_Details ?main_vw.entry_ty,'Use For Production',?item_vw.flotno,'',0.00,?main_vw.tran_cd,?item_vw.itserial"
					Endcase
					nretval = _curform.sqlconobj.dataconn([EXE],company.dbname,sq1,"_validatelot","_curform.nHandle",_curform.DataSessionId)
					If nretval <=0
						Return .F.
					Endif
					*!*				MESSAGEBOX("udappetvalid 5")
					If Used("_validatelot")
						*!*									MESSAGEBOX("udappetvalid 6")
						If nretval > 0 And Reccount('_validatelot') > 0
							*!*										MESSAGEBOX("udappetvalid 7")
							...
							If Used("Itref_vw")
								...
								If Reccount('curItref') > 0
									Update a Set a.balqty = (a.baljoqty - Iif(Isnull(b.aqty),0,b.aqty));
										From _validatelot a Left Outer Join curQty b On (a.flotno==b.flotno)
								Else
									Update a Set a.balqty = (a.balwojoqty - mcurQty) From  _validatelot a Where a.flotno = mlotno
								Endif
							Else
								Update a Set a.balqty = (a.balwojoqty - mcurQty) From  _validatelot a Where a.flotno = mlotno
							Endif
							...
							If item_vw.qty <= _validatelot.balqty
								Wait Window "Balance qty available in Lot No.," + Alltrim(Str(_validatelot.balqty,12,2)) Nowait
							Else
								=Messagebox("No balance qty available for selected Lot No.",0+16,vumess)
								Return .F.
							Endif
							If _curform.pcvtype="OC"
								Replace Item With _validatelot.Item, fshade With _validatelot.fshade;
									,fgrade With _validatelot.fgrade In item_vw
							Endif
						Else
							Messagebox("No Records found!!",0+64,vumess)
							oObj.Value = Val(oObj.Tag)
			...
			*!*				If Inlist(_curform.pcvtype,"W1")  	&&Commented by Priyanka B on 14022020 for Bug-33252
			If Inlist(_curform.pcvtype,"W1","W3","W4")  	&&Modified by Priyanka B on 14022020 for Bug-33252
				&&Added by Priyanka B on 11102019 for Bug-32747 Start
				If Empty(Alltrim(main_vw.flotno))
					Messagebox("Lot No. cannot be empty!!",0+16,vumess)
					Return .F.
				Endif
				&&Added by Priyanka B on 11102019 for Bug-32747 End
				If !Empty(Alltrim(main_vw.flotno))  &&Added by Priyanka B on 11102019 for Bug-32747
					sq1 = "Execute USP_GetLot_Details ?main_vw.entry_ty,?main_vw.party_nm,?main_vw.flotno,'',0,?main_vw.tran_cd,''"
					nretval = _curform.sqlconobj.dataconn([EXE],company.dbname,sq1,"_validatelot","_curform.nHandle",_curform.DataSessionId)
					If Used("_validatelot")
						If nretval > 0 And Reccount('_validatelot') > 0
							*!*								Update a Set a.balqty =a.qty From _validatelot a Where a.qty=(a.balwojoqty+a.baljoqty)
							Update a Set a.allocqty = a.joqty,a.balqty = (a.qty - a.joqty) From _validatelot a

							Select main_vw
							Replace fSentQty With _validatelot.balqty In main_vw
							Replace All item_vw.flotno With _validatelot.flotno In item_vw
						Else
							Messagebox("No Records found!!",0+64,vumess)
							oObj.Value = Val(oObj.Tag)
							If Used('_validatelot')
			...
			If Inlist(_curform.pcvtype,"W2")
				...
				&&Added by Priyanka B on 11102019 for Bug-32747 End
				If !Empty(Alltrim(main_vw.flotno))  &&Added by Priyanka B on 11102019 for Bug-32747
					sq1 = "Execute USP_GetLot_Details ?main_vw.entry_ty,'Use For Production',?main_vw.flotno,?main_vw.fsentitem,0,?main_vw.tran_cd,''"
					nretval = _curform.sqlconobj.dataconn([EXE],company.dbname,sq1,"_validatelot","_curform.nHandle",_curform.DataSessionId)
					If Used("_validatelot")
						If nretval > 0 And Reccount('_validatelot') > 0
							*!*								Update a Set a.balqty =a.qty From _validatelot a Where a.qty=(a.balwojoqty+a.baljoqty)
			...
			&&Added by Priyanka B on 10102019 for Bug-32747 Start
			If Inlist(_curform.pcvtype,"AF","FO")
				&&Added by Priyanka B on 11102019 for Bug-32747 Start
				If Empty(Alltrim(item_vw.flotno))
					Messagebox("Lot No. cannot be empty!!",0+16,vumess)
					Return .F.
				...
				&&Added by Priyanka B on 11102019 for Bug-32747 End
				If !Empty(Alltrim(item_vw.flotno))  &&Added by Priyanka B on 11102019 for Bug-32747
					sq1 = "select * from (select flotno,entry_ty,tran_cd,itserial from iiitem union all select flotno,entry_ty,tran_cd,itserial from aritem";
						+" union all select flotno,entry_ty,tran_cd,itserial from ositem) aa where flotno<>'' and flotno=RTRIM(LTRIM(?item_vw.flotno))"
					If _curform.editmode
						sq1 = sq1 + " and (entry_ty+quotename(tran_cd))<>'" + Alltrim(main_vw.entry_ty)+"["+Alltrim(Str(main_vw.tran_cd)) + "]'"
					Endif
					nretval = _curform.sqlconobj.dataconn([EXE],company.dbname,sq1,"_validatelot","_curform.nHandle",_curform.DataSessionId)
					If Used("_validatelot")
						If nretval > 0 And Reccount('_validatelot') > 0
							Messagebox("This Lot No. is already in use!!",0+64,vumess)
							oObj.Value = Alltrim(Iif(oObj.Tag="*#*",'',oObj.Tag))
							If Used('_validatelot')
								Use In _validatelot
							Endif
							Return .F.
				...
				Endif
			Endif
			&&Added by Priyanka B on 10102019 for Bug-32747 End
			If Used('_validatelot')
				Use In _validatelot
			Endif
		Endif
...
&&Added by Priyanka B on 25072019 for Bug-32747 End


&& Added by Shrikant S. on 14/09/2019 for Bug-32836			&& Start
Procedure CheckIsUnderSEZ
	...
	llImport= Iif(Inlist(Alltrim(tmp_type.supp_type),"SEZ")  ,.T.,.F.)
	If Used('tmp_type')
		Use In tmp_type
	Endif
	If nhandle >0
	...
	Endif
	Return llImport
	&& Added by Shrikant S. on 14/09/2019 for Bug-32836			&& End

	&&Added by Priyanka B on 30092019 for Bug-32747 Start
Procedure fUpdt_whn
	Lparameters oObj
	_curform = _Screen.ActiveForm
	oObj.Tag = Transform(oObj.Value)
	sq1 = "Select [type] as ittype from it_mast where it_code=?item_vw.it_code"
	nretval = _curform.sqlconobj.dataconn([EXE],company.dbname,sq1,"_it_type","_curform.nHandle",_curform.DataSessionId)
	Select _it_type
	If Used("_it_type") And Reccount('_it_type')>0
		Select _it_type
		If (Inlist(Alltr(Upper(_it_type.ItType)),'FINISHED','SEMI FINISHED'))
			Return .T.
		Else
			Return .F.
		Endif
...
Procedure fUpdtDSG_valid
	Lparameters oObj,oFldName
	_curform = _Screen.ActiveForm
	_etdatasessionid = _curform.DataSessionId
	_curform.nhandle=0
	_malias = Alias()
	...
	If [efabric] $ vchkprod
		If _curform.addmode Or _curform.editmode
			If Empty(oFldName)
				Messagebox("Please provide proper field name as second parameter in valid condition!!",0+64,vumess)
				Return.F.
			Endif
			oFldName1 = 'Item_vw.'+oFldName
			&&If Inlist(_curform.pcvtype,"AF","PY","ID","FO") And Type(oFldName1)<>'U' And !Empty(&oFldName1)  &&Commented by Prajakta B. on 08-01-2020 for Bug 33074
			If Inlist(_curform.pcvtype,"AF","PY","ID","FO","P6") And Type(oFldName1)<>'U' And !Empty(&oFldName1)  &&Modified by Prajakta B. on 08-01-2020 for Bug 33074
				Do Case
					Case Alltrim(Upper(oFldName))='FDESIGN'
						msqlstr = "SELECT DISTINCT [NAME] AS FDESIGN FROM TEXT_DESIGN_MASTER WHERE [NAME]=?item_vw.fdesign"
					Case Alltrim(Upper(oFldName))='FSHADE'
						msqlstr = "SELECT DISTINCT [NAME] AS FSHADE FROM TEXT_SHADE_MASTER WHERE [NAME]=?item_vw.fshade"
				Endcase
				nretval = _curform.sqlconobj.dataconn([EXE],company.dbname,msqlstr,"_cur1","_curform.nHandle",_curform.DataSessionId)
				If nretval <=0
					Return .F.
				Endif
				If Used("_cur1") And Reccount("_cur1")=0
					Messagebox("No records found!!",0+64,vumess)
					If Type('oObj.Value')<>'N'
						oObj.Value = Iif(oObj.Tag="*#*",'',oObj.Tag)
					Else
						oObj.Value = Iif(oObj.Tag="*#*",'',Val(Obj.Tag))
					Endif
				Endif
				If Used('_cur1')
					Use In _cur1
	...
	Endif
Endproc
&&Added by Priyanka B on 30092019 for Bug-32747 End
