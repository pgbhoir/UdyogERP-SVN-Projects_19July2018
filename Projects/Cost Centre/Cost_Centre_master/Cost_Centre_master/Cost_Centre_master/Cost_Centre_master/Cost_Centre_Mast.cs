...
namespace Cost_Centre_master
{
    public partial class Cost_Centre_Mast : uBaseForm.FrmBaseForm
    {
        ...
        {
            this.errorProvider1.Clear();  //Added by Priyanka B on 31/05/2017
            this.mthNew();
        ...
        public void ControlBind(string tableName)
        {
            ...
            //strSQL = "select * from cost_cen_mast_onCostExpand(" + pCompId.ToString() + ")"; //Birendra : Bug-8648 on 15/05/2013  //Commented by Priyanka B on 31/05/2017

            //Added by Priyanka B on 31/05/2017 Start
            if (this.pEditMode)
            {
                strSQL = "select * from cost_cen_mast_onCostExpand(" + pCompId.ToString() + ",'" + this.txtCostName.Text.ToString().Trim()
                    + "')";
            }
            else
            {
                strSQL = "select * from cost_cen_mast_onCostExpand(" + pCompId.ToString() + ",'')";
            }
            //Added by Priyanka B on 31/05/2017 End
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (save_valid())
            {
                ...
                MessageBox.Show("Record Saved Successfully", this.pPApplText, MessageBoxButtons.OK);  //Added by Priyanka B on 01/06/2017
        ...
        {
            //if (this.CmbCostUnder.Text.Length == 0)
            //if (this.CmbCostUnder.Text.Trim().Length == 0)   //Birendra : Bug-8356 on 17/01/2013  //Commented by Priyanka B on 01/06/2017
            if (this.CmbCostUnder.Text.Trim().Length == 0 && this.txtCostName.Text.Trim() != "")   //Added by Priyanka B on 01/06/2017
            {
                this.errorProvider1.SetError(this.CmbCostUnder, "Cost Centre Under Can Not Be Empty!!!");
                this.CmbCostUnder.Focus();
                return;
