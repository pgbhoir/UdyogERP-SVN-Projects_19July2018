    ...
    public class cls_Settings
    {
        ...
        public DataSet GetAllInitialData(int compid)
        {
            ...
            string sqlquery = "select code_nm,bcode_nm,entry_ty,applevel,applevel_init from lcode where apgenps = 1 " +
                            ...
                            //"union all " +
                            //Commented by Kishor A. for bug -21965 on 29-02-2016 "select 'Role' as type, user_roles as user_role,'' as user_name, '' as designation,'' as dept,'' as email,company,user_roles from vudyog..userroles " +
                            //Commented by Kishor A. for bug -21965 on 29-02-2016  "union all " +
            ...
            // Added By Kishor A. for Bug -21965 on 25-02-2016 Start            
            ...
            foreach (DataRow dr in m_ds_settings.Tables["user_mstr"].Rows)
            {
                ...
                if (lcompany == "0x")
                {
                    User_Role += Environment.NewLine + i + ". " + dr["user_roles"];
                    i++;
                }
            }
            if (User_Role != "")
            {
                _roleCompany = "Company name not found in the user roles setting for the below users. ";
                _roleCompany = _roleCompany + User_Role;
            }
            // Added By Kishor A. for Bug -21965 on 25-02-2016 End.
            ...
            foreach (DataRow dr in m_ds_settings.Tables["Usersroles"].Rows)
            {
                if (!string.IsNullOrEmpty(Convert.ToString(dr["company"])))
                {
                    ...
                    /* Commented By Kishor A. for Bug -21965 on 25-02-2016
                      if (cls_Common.Decrypt(lcompany, Convert.ToString(dr["user_roles"]).Trim()).IndexOf(compname) < 0) 
                      dr.Delete();
                     */
                    ...
                    {
                        throw new Exception(_roleCompany);
                    }
                    // Added By Kishor A. for Bug -21965 on 25-02-2016 End.
                }
            }
