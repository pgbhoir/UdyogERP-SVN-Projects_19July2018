...
namespace MLA.DL
{
    ...
    {
        public bool PvEmailExists = true; //Added By Kishor A. for Bug-21965 on-08-11-2016
        public cls_Approval()
        {
        }
                                       ...
                                       string userid)
        {
            ...
            //Added By Kishor A. for Bug-21965 on 30-03-2016 Start
            sqlquery = "EXECUTE USP_APPROVALWORKFLOW_MASTRS '" + userrole.Trim() + "','" + userid.Trim() + "','" + masttype + "'";

            //Commened By Kishor A. for Bug-21965 on 30-03-2016
            //if (masttype == "AM")
            ...
            //               "where /* a.levelstatus not in ('Approved','Rejected') -- Commneted By Kishor A for Bug-21965 on 06-01-2016 " +
            ...
            //Added By Kishor A. for Bug-21965 on 30-03-2016 End
                                      ...
                                      DateTime enddt, string userid)
        {
            ...
            string sDate = Convert.ToDateTime(startdt).ToString("dd/MM/yyyy");
            string eDate = Convert.ToDateTime(enddt).ToString("dd/MM/yyyy");

            //Added By Kishor A. for Bug-21965 on 30-03-2016 Start
            string sqlquery = "set dateformat dmy Execute USP_APPROVALWORKFLOW_DATA '" + tblname.Trim() + "','" + userrole.Trim() + "','" + userid.Trim() + "','" + trantype.Trim() + "','" +
                sDate + "','" + eDate + "'";

            //Commented for Bug-21965 on 30-03-2016 
            ...
            //                  "and -- Commneted By Kishor A for Bug-21965 on 06-01-2016*/ a.prelevelstatus = (case when b.bypasslevel = 0 then 'Approved' else '' end) " +
            //                  "and b.active = 1 " +
                         ...
                         string keyfld)
        {
            ...
            DataTable TempDataTbl = dtView.GetChanges(DataRowState.Modified); //Added By kishor A. for Bug-21965 on 06-01-2016
            //foreach (DataRow drow in dtView.Rows)
            foreach (DataRow drow in TempDataTbl.Rows)      // Changed by Sachin N. S. on 31/01/2017 for Bug-21965
            {
                    ...
                    Convert.ToString(drow["levelstatus"]).Trim() == "Rejected")
                {
                    ...
                    {
                        finalstring= finalstring+ " update applevel_dtl set levelstatus='Rejected' Where entry_ty = '" + Convert.ToString(drow["entry_ty"]).Trim() + "'" +
                                                    " and tran_cd = " + Convert.ToInt32(drow[keyfld]) + " and applevel ="+ (applevel+1); 
                    }
                    ...
                    if (prelevel > 0 || prelevel == 0)
                    {
                        if (Convert.ToString(drow["levelstatus"]).Trim() == "Rejected")
                        {
                                ...
                                foreach (DataRow rowAppLvlMstr in drapplevelMstr)
                                {
                                    if (Convert.ToString(rowAppLvlMstr["type"]) == "Role") // If type is role then mail has sent to all users which are under Role
                                    {
                                        ...
                                        foreach (DataRow drowRoles in drUser)
                                        {
                                            ...
                                            // Commneted By kishor A. for Bug-21965 on 06-01-2016 Start
                                            ...
                                            // Commneted By kishor A. for Bug-21965 on 06-01-2016 End

                                            //Added By kishor A. for Bug-21965 on 06-01-2016 Start
                                            string TmpTrancd = drow["tran_cd"].ToString();
                                            string TmpEntry_ty = drow["entry_ty"].ToString();
                                            foreach (DataRow TempRw in TempDataTbl.Rows)
                                            {
                                                if (TmpTrancd == TempRw["Tran_cd"].ToString() && TmpEntry_ty == TempRw["entry_ty"].ToString())
                                                {
                                                    if (Convert.ToString(drowRoles["user_email"]).Trim() == string.Empty)
                                                    {
                                                        MessageBox.Show("User email Id. not found in User master of this user :" + Convert.ToString(drowRoles["user"]).Trim() + Environment.NewLine + "Email will not be sent.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                                        PvEmailExists = false; //Added By Kishor A. for Bug-21965 on-08-11-2016
                                                    }
                                                    AddListRejectMails(trantype, codename, appmailsub, userid, drow, drLogUser, drowRoles, ListAppMailSetter);
                                                }
                                            }
                                    ...
                                    else
                                    {
                                        ...
                                        foreach (DataRow TempRw in TempDataTbl.Rows)
                                        {
                                            if (TmpTrancd == TempRw["Tran_cd"].ToString())
                                            {
                                                if (Convert.ToString(drUser["user_email"]).Trim() == string.Empty)
                                                    MessageBox.Show("User email Id. not found in User master of this user :" + Convert.ToString(drUser["user"]).Trim() + Environment.NewLine + "Email will not be sent.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information); // Added By Kishor A. for Bug-21965 on 06-01-2016 
                                                AddListRejectMails(trantype, codename, appmailsub, userid, drow, drLogUser, drUser, ListAppMailSetter);
                                                PvEmailExists = false; //Added By Kishor A. for Bug-21965 on-08-11-2016
                                            }
                                        }
                                        // Added By kishor A. for Bug-21965 on 06-01-2016 End
                                    }
                                }
                            }
                            else
                            {
                                ...
                                // Added by Sachin N. S. on 28/01/2017 for GST -- Start
                                DataRow drPstUser, drUser;
                                drPstUser = (from DataRow row in dtAppLevel_Mstr.Rows
                                             where Convert.ToInt16(row["applevel"]) == applevel         //Added by Shrikant S. on 26/06/2018 for Bug-30904
                                             select row).FirstOrDefault();
                                //where Convert.ToInt16(row["applevel"]) > applevel     //Commented by Shrikant S. on 26/06/2018 for Bug-30904
                                if (drPstUser == null)
                                    throw new Exception("Approval level not found in master..");

                                if (drPstUser != null)
                                {
                                    drUser = (from DataRow rowUser in m_ds_settings.Tables["User"].Rows
                                              where Convert.ToString(rowUser["user"]).ToUpper().Trim() == Convert.ToString(drPstUser["user_role"]).Trim().ToUpper()
                                              select rowUser).FirstOrDefault();
                                ...
                                // Added by Sachin N. S. on 28/01/2017 for GST -- End
                                ...
                                // Commented by Sachin N. S. on 28/01/2017 for Bug-21965 -- Start
                                //// Added By kishor A. for Bug-21965 on 06-01-2016 Start
                                //string TmpTrancd = drow["tran_cd"].ToString();
                                //foreach (DataRow TempRw in TempDataTbl.Rows)
                                //{
                                ...
                                //}
                                //// Added By kishor A. for Bug-21965 on 06-01-2016 End
                                // Commented by Sachin N. S. on 28/01/2017 for Bug-21965 -- End
                            }
                        }
                    }
                    ...
                    if (prelevel > 0)
                    {
                        if (Convert.ToString(drow["levelstatus"]).Trim() == "Approved")
                        {
                            ...
                            foreach (DataRow rowApplevel in drapplevelMstr)
                            {
                                ...
                                {
                                    // Added by Sachin N. S. on 31/01/2017 for Bug-21965 -- Start

                                    DataRow drPstUser, drUser;
                                    drPstUser = (from DataRow row in dtAppLevel_Mstr.Rows
                                                 where Convert.ToInt16(row["applevel"]) > applevel
                                                 select row).FirstOrDefault();
                                    ...
                                    if (drUser == null)
                                        throw new Exception("User not found in User master..");
                                    ...
                                    // Commented by Sachin N. S. on 31/01/2017 for Bug-21965 -- Start
                                    //drUser = (from DataRow rowUser in m_ds_settings.Tables["User"].Rows
                                    //          where Convert.ToString(rowUser["user"]).ToUpper().Trim() == Convert.ToString(rowApplevel["user_role"]).Trim().ToUpper()
                                    //          select rowUser).FirstOrDefault();
                                    ...
                                    //    if (TmpTrancd == TempRw["Tran_cd"].ToString())
                                    //    {
                                    //        if (Convert.ToString(drUser["user_email"]).Trim() == string.Empty) // Test1
                                    //        {
                                    //            MessageBox.Show("User email Id. not found in User master of this user :" + Convert.ToString(drUser["user"]).Trim() + Environment.NewLine + "Email will not be sent.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                    //            PvEmailExists = false; //Added By Kishor A. for Bug-21965 on-08-11-2016
                                    //        }
                                    //        AddListNextApproveMails(trantype, codename, appmailsub, userid, strmailline, drow, drLogUser, drUser, ListAppMailSetter);
                                    ...
                                    //}
                                    //// Added By kishor A. for Bug-21965 on 06-01-2016 End
                                    // Commented by Sachin N. S. on 31/01/2017 for Bug-21965 -- End
                                ...
                                else
                                {
                                    ...
                                    if (Convert.ToString(rowApplevel["type"]) == "Role")
                                    {
                                        ...
                                        if (drPstUser != null)
                                        {
                                            drUser1 = (from DataRow rowUser in m_ds_settings.Tables["User"].Rows
                                                      where Convert.ToString(rowUser["user"]).ToUpper().Trim() == Convert.ToString(drPstUser["user_role"]).Trim().ToUpper()
                                                      select rowUser).FirstOrDefault();
                                            ...
                                            if (Convert.ToString(drUser1["user_email"]).Trim() == string.Empty)
                                            {
                                                MessageBox.Show("User email Id. not found in User master of this user :" + Convert.ToString(drUser1["user"]).Trim() + Environment.NewLine + "Email will not be sent.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information); // Added By kishor A. for Bug-21965 on 06-01-2016 
                                                PvEmailExists = false;
                                            }
                                            AddListRejectMails(trantype, codename, appmailsub, userid, drow, drLogUser, drUser1, ListAppMailSetter);
                                        }
                                    }
                                }
                            }
                                ...
                                foreach (DataRow rowAppLvlMstr in drapplevelMstr)
                                {
                                    ...
                                    if (Convert.ToString(rowAppLvlMstr["type"]) == "Role")
                                    {
                                        ...
                                        foreach (DataRow drowRoles in drUser)
                                        {
                                            ...
                                            if (Convert.ToString(drowRoles["user_email"]).Trim() == string.Empty)
                                            {
                                                MessageBox.Show("User email Id. not found in User master of this user :" + Convert.ToString(drowRoles["user"]).Trim() + Environment.NewLine + "Email will not be sent.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information); // Added By kishor A. for Bug-21965 on 06-01-2016 
                                                PvEmailExists = false; //Added By Kishor A. for Bug-21965 on-08-11-2016
                                            }
                                            // throw new Exception("User email Id. not found in User master of this user :" + Convert.ToString(drowRoles["user"]).Trim()); // Commented By kishor A. for Bug-21965 on 06-01-2016 
                                    ...
                                    else
                                    {
                                        ...
                                        // throw new Exception("User email Id. not found in User master of this user :" + Convert.ToString(drUser["user"]).Trim());// Commented By kishor A. for Bug-21965 on 06-01-2016
