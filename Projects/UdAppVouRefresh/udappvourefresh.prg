...
If Type('Lcode_vw.UnitBc')='L'
	If Lcode_vw.UnitBc=.T.
		If Type('_curvouobj._unitbcserialize')='O'
			_curvouobj._unitbcserialize._uetrigvourefresh()
			If Type('_curvouobj.cmdscanbc')='O'
				If Lcode_vw.Inv_stk='+'
					_curvouobj.cmdscanbc.Enabled=Iif(_curvouobj.addmode Or _curvouobj.editmode,.F.,.T.)
				Else
					_curvouobj.cmdscanbc.Enabled=Iif(_curvouobj.addmode Or _curvouobj.editmode,.T.,.F.)
				Endif
				_curvouobj.cmdscanbc.Refresh()
			Endif
		Endif
	Endif
Endif
