...
Function FindProfitOrLoss
	*************************
	...
	&& Find GP
	**** Changed by Sachin N. S. on 29/07/2021 for Bug-34643 -- Start
*!*		Select Sum(a.ClBal) As TrdExpense;
*!*			FROM _CTBAcMast a;
*!*			WHERE a.LevelFlg = Mtexpenseflg;
*!*			INTO Cursor CurTrdIncome

	Select Sum(a.ClBal-a.PT_opening) As TrdExpense;
		FROM _CTBAcMast a;
		WHERE a.LevelFlg = Mtexpenseflg;
		INTO Cursor CurTrdIncome
**** Changed by Sachin N. S. on 29/07/2021 for Bug-34643 -- End

	Go Top
	mTrdExpense = Iif(Isnull(TrdExpense),0,TrdExpense)				&& Debited

*!*		Messagebox("mTrdExpense : "+Transform(mTrdExpense))
...
*!*			INTO Cursor CurTrdIncome
	...
	Select Sum(a.ClBal-a.Pt_opening) As TrdIncome;
		FROM _CTBAcMast a;
		WHERE a.LevelFlg = Mtincomeflg;
		INTO Cursor CurTrdIncome
**** Changed by Sachin N. S. on 29/07/2021 for Bug-34643 -- End
...
*!*		Messagebox("mTrdIncome : "+Transform(mTrdIncome))
...
*!*		Messagebox("mMfgExpense : "+Transform(mMfgExpense))
**** Changed by Sachin N. S. on 29/07/2021 for Bug-34643 -- Start
*!*		Select Sum(a.ClBal) As TrdIncome;
*!*			FROM _CTBAcMast a;
...
*!*		Messagebox("mMfgIncome : "+Transform(mMfgIncome))
...
*!*		Messagebox("mIncome : "+Transform(mIncome))
*!*		Messagebox("mExpense : "+Transform(mExpense))
...
*!*			INTO Cursor CurIncome
	...
	Select Sum(a.ClBal-a.pt_opening) As Expense;
		FROM _CTBAcMast a;
		WHERE a.LevelFlg = Mexpenseflg;
		INTO Cursor CurIncome
...
*!*			INTO Cursor CurIncome
	...
	Select Sum(a.ClBal-a.Pt_opening) As Income;
		FROM _CTBAcMast a;
		WHERE a.LevelFlg = Mincomeflg;
		INTO Cursor CurIncome
**** Changed by Sachin N. S. on 29/07/2021 for Bug-34643 -- End
...
*!*		Messagebox("PandLGp : "+Transform(PandLGp))
	...
	*****================================================================*****
	***** Added by Sachin N. S. on 24/07/2020 for Bug-33603 -- Start

	Store 0 To PandLGp,mTrdIncome,mTrdExpense,mExpense,mIncome,mMfgIncome,mMfgExpense

	Select Sum(a.PT_Opening) As TrdExpense;
		FROM _CTBAcMast a;
		WHERE a.LevelFlg = Mtexpenseflg;
		INTO Cursor CurTrdIncome
	Go Top
	mTrdExpense = Iif(Isnull(TrdExpense),0,TrdExpense)

*!*		Messagebox("mTrdExpense : "+Transform(mTrdExpense))

	Select Sum(a.PT_Opening) As TrdIncome;
		FROM _CTBAcMast a;
		WHERE a.LevelFlg = Mtincomeflg;
		INTO Cursor CurTrdIncome
	Go Top
...
*!*		Messagebox("mTrdIncome : "+Transform(mTrdIncome))
	...
	Select Sum(a.PT_Opening) As TrdExpense;
		...
		INTO Cursor CurTrdIncome
	Go Top
	mMfgExpense = mTrdExpense + Iif(Isnull(TrdExpense),0,TrdExpense)
...
*!*		Messagebox("mMfgExpense : "+Transform(mMfgExpense))
	...
	Select Sum(a.PT_Opening) As TrdIncome;
		...
		INTO Cursor CurTrdIncome
	Go Top
	mMfgIncome = mTrdIncome + Iif(Isnull(TrdIncome),0,TrdIncome)
	mMfgIncome = -mMfgIncome
...
*!*		Messagebox("mMfgIncome : "+Transform(mMfgIncome))
	...
	Do Case
		...
		Case mMfgExpense > mMfgIncome			&& EXPENSE IS GREATER THAN INCOME
			m.diff = mMfgExpense-mMfgIncome
			mExpense = Abs(m.diff)
		Otherwise
			mIncome  = 0
...
*!*		Messagebox("mExpense : "+Transform(mExpense))
	...
	&& Find NP
	Select Sum(a.PT_Opening) As Expense;
		FROM _CTBAcMast a;
		WHERE a.LevelFlg = Mexpenseflg;
		INTO Cursor CurIncome
	Go Top
...
*!*		Messagebox("mExpense : "+Transform(mExpense))
	...
	Select Sum(a.PT_Opening) As Income;
		FROM _CTBAcMast a;
		WHERE a.LevelFlg = Mincomeflg;
		INTO Cursor CurIncome
	Go Top
...
*!*		Messagebox("PandLGp : "+Transform(PandLGp))
	...
	If Found()
		...
		Scatter Memvar
		Append Blank
		m.Ac_name='NET PROFIT & LOSS (Opening)'
		Gather Memvar
		Replace _CTBAcMast.debit With Iif(PandLGp>0,PandLGp,0)
		Replace _CTBAcMast.credit With Iif(PandLGp<0,Abs(PandLGp),0)
		Replace _CTBAcMast.ClBal With _CTBAcMast.debit-_CTBAcMast.credit
	...
	***** Added by Sachin N. S. on 24/07/2020 for Bug-33603 -- End
	*****================================================================*****
