...
If Vartype(uexapps) <> 'C' Or Empty(uexapps)
	=Messagebox('In Configuration file Xfile Path cannot be empty',16,[Udyog ERP])
	Return .F.
Else
	If !File(uexapps)
		=Messagebox('In Configuration file Specify application file is not found',16,[Udyog ERP])
		Return .F.
	Endif
