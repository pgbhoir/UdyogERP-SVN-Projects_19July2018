...
Do While custinfosrno <= 2
	...
	nFile	= Adir(arrFile,custinfofilenm+"*info.inf")			&& Added by Shrikant S. on 18/04/2019 for Registration
	_lens = 1
...
&& Added by Shrikant S. on 18/04/2019 for Registration 		&& Start
	If Reccount('custinfo_vw')>0
		lcprod=GlobalObj.getPropertyVal("ProductTitle")

		Select custinfo_vw
		Locate
&& Added by Shrikant S. on 26/06/2019 for AU 2.1.3		&& Start
		If Alltrim(custinfo_vw.prodnm)=="Visual Udyog GST"
			lcprod="Visual Udyog GST"
		Endif
&& Added by Shrikant S. on 26/06/2019 for AU 2.1.3		&& End
		If Alltrim(custinfo_vw.prodnm)!=Alltrim(lcprod)
*Delete From custinfo_vw

			Select custinfofile_vw
			Loop
		Endif
	Endif
&& Added by Shrikant S. on 18/04/2019 for Registration 		&& End
