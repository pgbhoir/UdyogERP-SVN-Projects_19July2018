...
If Used('Gen_miss_vw')
	...
	Use In Gen_miss_vw
Endif
sql_con = oSqlConObj.DataConn([EXE],vcDbName," Select * from "+ALLTRIM(nDbName)+"..Gen_inv with (NOLOCK) where 1=0 ",;
	[Gen_inv_vw],vnHandle,nDataSessionId,.F.)
If sql_con > 0
	sql_con = oSqlConObj.DataConn([EXE],vcDbName," Select * from "+ALLTRIM(nDbName)+"..Gen_miss with (NOLOCK) where 1=0 ",;
...
If sql_con > 0 And Used('Gen_inv_vw') And Used('Gen_miss_vw')
	...
	Do While .T.
		Do Case
		Case v_i_s_type = 'DAYWISE'
			sql_con = oSqlConObj.DataConn([EXE],vcDbName," Select Top 1 Inv_no from "+ALLTRIM(nDbName)+"..Gen_inv with (TABLOCKX) ;
		...
		If sql_con > 0 And Used('tmptbl_vw')
			...
			If sql_con > 0
				Do Case
				Case v_i_s_type = 'DAYWISE'
					sql_con = oSqlConObj.DataConn([EXE],vcDbName," Select Top 1 Flag from "+ALLTRIM(nDbName)+"..Gen_miss where ;
