...
If Empty(vInvoiceSeries)
	vInvoiceSeries=Space(Len(Main_vw.Inv_sr))
Endif
If Empty(voldInvoiceSeries)
	voldInvoiceSeries=Space(Len(Main_vw.Inv_sr))
Endif
			...
			If sql_con > 0
				...
				If vFoundInMiss='N'
					...
					If sql_con > 0
						...
						minv_no = Padr(v_i_prefix+v_i_middle+minv_no+v_i_suffix,Len(Main_vw.Inv_no),' ')
