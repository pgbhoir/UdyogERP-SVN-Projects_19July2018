...
&&Added by Priyanka B on 24082019 for Bug-32361 Start

oFrm=_Screen.ActiveForm
_datasessionid=_Screen.ActiveForm.DataSessionId
If (Inlist(oFrm.pcvtype,"GC","GD","SR","PR") Or Inlist(oFrm.behave,"GC","GD","SR","PR")) And ("vugst" $ vchkprod)
	If oFrm.Addmode Or oFrm.Editmode
		If Used('Itref_vw')
			Select Itref_vw
			mrecno=Iif(!Eof(),Recno(),0)
	...
	Endif
Endif
&&Added by Priyanka B on 24082019 for Bug-32361 End

&&Added by Priyanka B on 22082019 for Bug-32747 Start
oFrm=_Screen.ActiveForm
_datasessionid=_Screen.ActiveForm.DataSessionId
If (Inlist(oFrm.pcvtype,"ID") And ("efabric" $ vchkprod))
	...
	If (oFrm.Addmode Or oFrm.Editmode)
		If Type('Item_vw.FRECQTY')<>'U' And Type('Item_vw.FSHEXQTY')<>'U'
			...
			If Used('Itref_vw')
				...
				Scan For Inlist(Itref_vw.rentry_ty,"AF")

					Select item_vw
					Scan For ItSerial==Itref_vw.ItSerial
						Replace item_vw.FRECQTY With item_vw.qty In item_vw
						Replace item_vw.FSHEXQTY With (item_vw.qty - item_vw.FRECQTY) In item_vw
					Endscan
					Select Itref_vw
				Endscan
	...
	Endif
Endif
&&Added by Priyanka B on 22082019 for Bug-32747 End

&&Added by Prajakta B. on 26/09/2019 for Bug 32845  --Start
oFrm=_Screen.ActiveForm
_datasessionid=_Screen.ActiveForm.DataSessionId
If Inlist(oFrm.pcvtype,"PI")
	If (oFrm.Addmode Or oFrm.Editmode)
		If !Used("PayTerms_vw")
			sql_con = oFrm.SqlConObj.DataConn([EXE],Company.DbName,[Select * From Payterms],[PayTerms_vw],;
				"This.Parent.nHandle",_datasessionid,.F.)
			If sql_con<0
				Return .F.
			Endif
			Update PayTerms_vw Set Checked=.F. Where VALIDIN Like '%'+main_Vw.entry_ty+'%'
		Endif
		
		If !Used("PayTermsdet_vw")
			lcstr = "Select * From Paytermsdet"
			sql_con = oFrm.SqlConObj.DataConn([EXE],Company.DbName,lcstr,[PayTermsdet_vw],;
				"This.Parent.nHandle",_datasessionid,.F.)
			If sql_con<0
				Return .F.
			Endif
		...
		If Used('Itref_vw')
			...
			Scan
				...
				Go Top
				Scan
					If tran_cd==Itref_vw.itref_tran And entry_ty== Itref_vw.rentry_ty
						Select PayTerms_vw
						Go Top
						Locate For (PayTermsdet_vw.entry_ty $ PayTerms_vw.VALIDIN And main_Vw.entry_ty $ PayTerms_vw.VALIDIN) ;
							And Alltrim(Upper(PayTermsdet_vw.ptname)) == Alltrim(Upper(PayTerms_vw.ptname))
						If Found()
							Replace Checked With .T. In PayTerms_vw
						Endif
						Select PayTermsdet_vw
					Endif
				Endscan
	...
	Endif
Endif
&&Added by Prajakta B. on 26/09/2019 for Bug 32845  --End

*-- Added By Anil A on Date 12-02-2020 for Bug 33252 Start

IF INLIST(main_Vw.entry_ty,"I7","I8")	
	SELECT Header
	Replace cons_id  WITH main_vw.ac_id
	SELECT main_vw
	Replace main_vw.cons_id WITH main_vw.ac_id IN main_vw
ENDIF
*-- Added By Anil A on Date 12-02-2020 for Bug 33252 End
