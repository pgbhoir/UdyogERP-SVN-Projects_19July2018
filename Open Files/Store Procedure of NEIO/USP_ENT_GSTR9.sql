...
BEGIN
	...
	SET @IGST_AMT =0.00
	SET @CESS_AMT =0.00
	--Commented by Priyanka B on 24092019 for AU 2.2.2 Start
	...
	--Commented by Priyanka B on 24092019 for AU 2.2.2 End
	...
	--AND SUPP_TYPE not in ('Unregistered')  --Commented by Priyanka B on 24092019 for AU 2.2.2
	...
	--AND GSTIN NOT IN ('Unregistered') --Commented by Priyanka B on 24092019 for AU 2.2.2
...
from (
	...
	and a.entry_all<>'UB'
	and ((a.ENTRY_ALL+ quotename(a.Main_tran)) not in (select  a.entry_ty+ quotename(a.Tran_cd)  from #PTEPRCMTBL a LEFT OUTER JOIN shipto c ON (c.shipto_id = a.scons_id) where c.Supp_type in('import','sez','Eou','','Export')) and (a.ENTRY_ALL+ quotename(a.Main_tran)) not in (select  a.entry_ty+ quotename(a.Tran_cd)  from #BPRCM a LEFT OUTER JOIN AC_MAST  c ON (c.Ac_id  = a.Ac_id ) where c.Supp_type in('import','sez','Eou','','Export')))
	and (d.cgsrt_amt1+d.sgsrt_amt1+d.igsrt_amt1) > 0  --Added by Priyanka B on 12112019 for Bug-32361
	...
	FROM #GSTR9TBL WHERE mEntry IN('PT','PR','CN','DN') 
	AND Isservice  ='GOODS'
	--AND ((st_type ='OUT OF COUNTRY' AND SUPP_TYPE IN('IMPORT','')) OR ( st_type IN('INTRASTATE','INTERSTATE') AND SUPP_TYPE IN('SEZ','IMPORT','EOU','EXPORT')))   --Commented by Priyanka B on 12112019 for Bug-32361
	AND ((st_type IN ('OUT OF COUNTRY','INTRASTATE','INTERSTATE') AND SUPP_TYPE IN('SEZ','IMPORT','EOU','EXPORT','')))   --Modified by Priyanka B on 12112019 for Bug-32361
	...
	*/
	...
	UNION ALL
		...
		--AND b.date <= @EDATE  --Commented by Priyanka B on 13112019 for Bug-32361
		AND b.date < @SDATE  --Modified by Priyanka B on 13112019 for Bug-32361
		...
		delete from #TBL1 where (entry_ty='GD' and amt_ty='DR') or (entry_ty='GC' and amt_ty='CR') --Added by Priyanka B on 25092019 for AU 2.2.2
	...
	Insert into #GSTR9(PART,PARTSR,SRNO,HSNCODE,uqc,totqty,taxableamt,taxrate,CGST_AMT,SGST_AMT,IGST_AMT,Cess_Amt)
		SELECT * FROM (
		  ...
		  --,gstrate  --Commented by Priyanka B on 16122019 for Bug-33113
		  ,(CGST_PER+SGST_PER+IGST_PER) as gstrate --Modified by Priyanka B on 16122019 for Bug-33113
		...
		and HSNCODE <>'' 
		group by HSNCODE,SERVTCODE,Isservice,uqc
		--,gstrate  --Commented by Priyanka B on 16122019 for Bug-33113
		,(CGST_PER+SGST_PER+IGST_PER) --Modified by Priyanka B on 16122019 for Bug-33113
		)AA order by srno
	...
	--where (A.SUPP_TYPE <> 'unregistered' AND (A.mEntry In('PT','PR','CN','DN','EP') OR A.Entry_ty = 'UB'))   --Commented by Priyanka B on 25092019 for AU 2.2.2
	where (A.mEntry In('PT','PR','CN','DN','EP'))   --Modified by Priyanka B on 25092019 for AU 2.2.2
	and A.againstgs in ('PURCHASES','')
