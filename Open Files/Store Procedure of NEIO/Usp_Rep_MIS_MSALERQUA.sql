...
Into #tmpFlds 
From Lother Where e_code='SQ' and DISP_MIS=1
union all
...
Where Entry_ty='SQ' and DISP_MIS=1
union all
Select case when att_file=1 then 'SQMAIN.'+RTRIM(pert_name) else 'SQITEM.'+RTRIM(pert_name) end as flds,FLD_NM=RTRIM(pert_name),att_file,data_ty='',head_nm= (rtrim(head_nm)+'PER')  From DcMast
Where Entry_ty='SQ' and DISP_MIS=1
...
SET @SQLCOMMAND=''
--SET @SQLCOMMAND = N''+@QueryString+''+' into '+'##main11'+' FROM SQMAIN'		--Commenetd by Divyang P on 17032020 for Bug-33349
SET @SQLCOMMAND = N''+@QueryString+''+N''+@Tot_flds+''+' into '+'##main11'+' FROM SQMAIN' 
 ...
 SET @SQLCOMMAND =	@SQLCOMMAND+'and (SQMAIN.inv_sr BETWEEN '+CHAR(39)+cast(@FSeries as varchar)+CHAR(39)+'  AND '+CHAR(39)+cast(@TSeries as varchar )+CHAR(39)+') and (SQMAIN.Dept BETWEEN '+CHAR(39)+@FDept+CHAR(39)+' AND '+CHAR(39)+@TDept+CHAR(39)+')'
 SET @SQLCOMMAND =	@SQLCOMMAND+'and (SQMAIN.cate BETWEEN '+CHAR(39)+cast(@FCategory as varchar)+CHAR(39)+'  AND '+CHAR(39)+cast(@TCategory as varchar )+CHAR(39)+') and (SQitem.ware_nm BETWEEN '+CHAR(39)+@FWarehouse+CHAR(39)+' AND '+CHAR(39)+@TWarehouse+CHAR(39)+')'
 SET @SQLCOMMAND =	@SQLCOMMAND+' ORDER BY SQMAIN.INV_SR,SQMAIN.INV_NO'
 print @sqlcommand
execute sp_executesql @SQLCOMMAND

--SET @SQLCOMMAND ='select Date,RDATE,inv_no,RINV_NO,Party_nm,GSTIN,State,st_type,item,HSNCODE,qty,stkunit,rate,Total,u_asseamt,CGST_PER,CGST_AMT,SGST_PER,SGST_AMT,IGST_PER,IGST_AMT,TotalGST,IT_GROAMT,net_amt from ##main11'   Commenetd by Divyang P on 17032020 for Bug-33349

--Added by Divyang P on 17032020 for Bug-33349  Start
SET @SQLCOMMAND=''
SET @SQLCOMMAND ='select a.Date,a.RDATE,a.inv_no,a.RINV_NO,a.Party_nm,a.GSTIN,State,st_type,a.item,HSNCODE,a.qty,a.stkunit,a.rate,a.Total,a.u_asseamt,a.CGST_PER,a.CGST_AMT,a.SGST_PER,a.SGST_AMT,a.IGST_PER,a.IGST_AMT,a.TotalGST,a.IT_GROAMT,a.net_amt '
SET @SQLCOMMAND = @SQLCOMMAND + N''+@Tot_flds+''+ ' from ##main11 a inner join SQMAIN on (SQMAIN.tran_cd=a.tran_cd) inner join SQITEM on (SQITEM.tran_cd=SQMAIN.tran_cd) '
--Added by Divyang P on 17032020 for Bug-33349  END
