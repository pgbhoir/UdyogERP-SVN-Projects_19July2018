If Exists(Select [name] From SysObjects Where xType='P' and [name]='Usp_Rep_MIS_PTR')
Begin
	Drop Procedure Usp_Rep_MIS_PTR
End
GO
...
Create PROCEDURE [dbo].[Usp_Rep_MIS_PTR] 
...
Into #tmpFlds 
From Lother Where e_code in ('PT') and DISP_MIS=1
union all
...
Where Entry_ty in ('PT') and DISP_MIS=1
...
Where Entry_ty='PT' and DISP_MIS=1
--Added by Divyang P on 18032020 for Bug-33349  End




SET @QueryString ='Select [Financial Year]=ptmain.l_yn,YearMonth=convert(varchar(4),year(ptmain.date))+''-''+convert(varchar(2),ptmain.date,101),[Month]=left(datename(month,ptmain.date),3)+''-''+right(year(ptmain.date),2),    --Modified by Divyang for Tkt-33129 DT:18/12/2019   fld:l_yn,YearMonth,Month,Location
AC_NAME=(CASE WHEN PTMAIN.sac_id> 0 THEN (CASE WHEN ISNULL(SHIPTO.MailName,'''')='''' THEN AC_MAST.mailname ELSE SHIPTO.mailname END) ELSE AC_MAST.mailname END)
...
SET @QueryString =@QueryString+',ST_TYPE=CASE WHEN PTMAIN.SCONS_ID >0 THEN SHIPTO1.ST_TYPE ELSE AC_MAST.ST_TYPE END,PTMAIN.EWBN,PTMAIN.EWBDT,PTMAIN.EWBDIST,PTMAIN.roundoff,PTITEM.LINERULE,PTITEM.CCESSRATE,Location=shipto.location_id,CESSReceivable='+@CESSReceivable+''
...
SET @SQLCOMMAND =	@SQLCOMMAND+' WHERE  (ptmain.date BETWEEN '+CHAR(39)+cast(@FrmDate as varchar)+CHAR(39)+'  AND '+CHAR(39)+cast(@Todate as varchar )+CHAR(39)+') and (ptmain.Party_nm BETWEEN '+CHAR(39)+@FParty+CHAR(39)+' AND '+CHAR(39)+@TParty+CHAR(39)+')'
SET @SQLCOMMAND =	@SQLCOMMAND+'and (ptmain.inv_sr BETWEEN '+CHAR(39)+cast(@FSeries as varchar)+CHAR(39)+'  AND '+CHAR(39)+cast(@TSeries as varchar )+CHAR(39)+') and (ptmain.Dept BETWEEN '+CHAR(39)+@FDept+CHAR(39)+' AND '+CHAR(39)+@TDept+CHAR(39)+')'
SET @SQLCOMMAND =	@SQLCOMMAND+'and (ptmain.cate BETWEEN '+CHAR(39)+cast(@FCategory as varchar)+CHAR(39)+'  AND '+CHAR(39)+cast(@TCategory as varchar )+CHAR(39)+') and (ptitem.ware_nm BETWEEN '+CHAR(39)+@FWarehouse+CHAR(39)+' AND '+CHAR(39)+@TWarehouse+CHAR(39)+')'
SET @SQLCOMMAND= @SQLCOMMAND+' Order by item_no' 
print @sqlcommand
execute sp_executesql @sqlcommand

SET @SQLCOMMAND ='select [Financial Year],YearMonth,[Month],a.date,a.inv_no,a.Pinvno,a.Pinvdt,a.ST_TYPE,a.party_nm,a.GSTIN,a.state,a.item,a.HSNCODE,a.qty,a.rateunit,a.rate,total=(a.qty*a.rate),a.ITEMTAX,a.U_ASSEAMT'   --Modified by Divyang for Tkt-33129 DT:18/12/2019   fld:l_yn,YearMonth,Month,Location
SET @SQLCOMMAND=@SQLCOMMAND+',a.CGST_PER,a.CGST_AMT,a.SGST_PER,a.SGST_AMT,a.IGST_PER,a.IGST_AMT'
SET @SQLCOMMAND=@SQLCOMMAND+',TotalGST=a.CGST_AMT+a.SGST_AMT+a.IGST_AMT,a.EWBN,a.EWBDT, a.EWBDIST,a.ITAddLess as discount,a.roundoff,a.v_gro_amt,a.ITEMNTX,a.net_amt ,'
SET @SQLCOMMAND=@SQLCOMMAND+'a.CGSTReceivable,a.SGSTReceivable,a.IGSTReceivable,U_FREIGHT='''',a.LINERULE,PKGFRWD='''',CCESSRATE=case when a.CCESSRATE=''NO-CESS'' then '''' else a.CCESSRATE end ,a.COMPCESS,a.Location,a.CESSReceivable '
SET @SQLCOMMAND = @SQLCOMMAND + N''+@Tot_flds+''+ ' from ##main11 a inner join PTMAIN on (PTMAIN.tran_cd=a.tran_cd) inner join ptitem on (ptitem.tran_cd=PTMAIN.tran_cd) '		--Added by Divyang P on 18032020 for Bug-33349
print @SQLCOMMAND
execute sp_executesql @SQLCOMMAND
drop table ##main11
