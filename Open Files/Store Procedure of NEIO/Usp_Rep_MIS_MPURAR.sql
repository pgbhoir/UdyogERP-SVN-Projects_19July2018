...
Select case when att_file=1 then 'ARMAIN.'+RTRIM(FLD_NM) else 'ARITEM.'+RTRIM(FLD_NM) end as flds,FLD_NM=RTRIM(FLD_NM),att_file,data_ty,head_nm
Into #tmpFlds 
From Lother Where e_code in ('AR') and DISP_MIS=1
union all
Select case when att_file=1 then 'ARMAIN.'+RTRIM(FLD_NM) else 'ARITEM.'+RTRIM(FLD_NM) end as flds,FLD_NM=RTRIM(FLD_NM),att_file,data_ty='',head_nm  From DcMast
Where Entry_ty in ('AR') and DISP_MIS=1
--Added by Divyang P on 18032020 for Bug-33349  Start
union all
Select case when att_file=1 then 'ARMAIN.'+RTRIM(pert_name) else 'ARITEM.'+RTRIM(pert_name) end as flds,FLD_NM=RTRIM(pert_name),att_file,data_ty='',head_nm= (rtrim(head_nm)+'PER')  From DcMast
Where Entry_ty='AR' and DISP_MIS=1
...
SET @SQLCOMMAND =	@SQLCOMMAND+'and (armain.inv_sr BETWEEN '+CHAR(39)+cast(@FSeries as varchar)+CHAR(39)+'  AND '+CHAR(39)+cast(@TSeries as varchar )+CHAR(39)+') and (armain.Dept BETWEEN '+CHAR(39)+@FDept+CHAR(39)+' AND '+CHAR(39)+@TDept+CHAR(39)+')'
SET @SQLCOMMAND =	@SQLCOMMAND+'and (armain.cate BETWEEN '+CHAR(39)+cast(@FCategory as varchar)+CHAR(39)+'  AND '+CHAR(39)+cast(@TCategory as varchar )+CHAR(39)+') and (aritem.ware_nm BETWEEN '+CHAR(39)+@FWarehouse+CHAR(39)+' AND '+CHAR(39)+@TWarehouse+CHAR(39)+')'

SET @SQLCOMMAND= @SQLCOMMAND+' Order by item_no' 
print @sqlcommand
execute sp_executesql @sqlcommand

SET @SQLCOMMAND ='select a.date,a.inv_no,a.Pinvno,a.Pinvdt,a.ST_TYPE,a.party_nm,a.GSTIN,a.state,a.item,a.HSNCODE,a.qty,a.rateunit,a.rate,total=(a.qty*a.rate),a.ITEMTAX,a.U_ASSEAMT'
...
SET @SQLCOMMAND=@SQLCOMMAND+'a.CGSTReceivable,a.SGSTReceivable,a.IGSTReceivable,U_FREIGHT='''',a.LINERULE,PKGFRWD='''',CCESSRATE=case when a.CCESSRATE=''NO-CESS'' then '''' else a.CCESSRATE end ,a.COMPCESS,a.CESSReceivable '
SET @SQLCOMMAND = @SQLCOMMAND + N''+@Tot_flds+''+ ' from ##main11 a inner join ARMAIN on (ARMAIN.tran_cd=a.tran_cd) inner join ARITEM on (ARITEM.tran_cd=ARMAIN.tran_cd) '
execute sp_executesql @SQLCOMMAND
drop table ##main11
