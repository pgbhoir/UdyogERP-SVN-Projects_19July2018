If Exists(Select [name] From SysObjects Where xType='P' and [name]='Usp_Rep_MIS_MISSUE4')
Begin
	Drop Procedure Usp_Rep_MIS_MISSUE4
End
GO
...
Create PROCEDURE [dbo].[Usp_Rep_MIS_MISSUE4] 
...
Into #tmpFlds 
From Lother Where e_code='LI' and DISP_MIS=1
union all
...
Where Entry_ty='LI' and DISP_MIS=1
union all
...
SET @SQLCOMMAND=''
--SET @SQLCOMMAND = N''+@QueryString+''+N''+' into '+'##main11'+' FROM iimain'	--Commenetd by Divyang P on 18032020 for Bug-33349  
SET @SQLCOMMAND = N''+@QueryString+''+N''+@Tot_flds+''+' into '+'##main11'+' FROM iimain' --Added by Divyang P on 18032020 for Bug-33349  
 ...
 SET @SQLCOMMAND =	@SQLCOMMAND+'WHERE iimain.entry_ty=''LI'' and (iimain.date BETWEEN '+CHAR(39)+cast(@FrmDate as varchar)+CHAR(39)+'  AND '+CHAR(39)+cast(@Todate as varchar )+CHAR(39)+') and (iimain.Party_nm BETWEEN '+CHAR(39)+@FParty+CHAR(39)+' AND '+CHAR(39)+@TParty+CHAR(39)+')'
...
--exec sp_executesql N'execute Usp_Rep_MIS_MISSUE4 @FDate,@TDate,@FParty,@TParty',N'@FParty nvarchar(4000),@TParty nvarchar(10),@FDate nvarchar(10),@TDate nvarchar(10)',@FParty=N'',@TParty=N'WORK ORDER',@FDate=N'04/01/2019',@TDate=N'03/31/2020'
--go
