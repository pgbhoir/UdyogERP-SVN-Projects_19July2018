...
Select case when att_file=1 then (CASE WHEN RTRIM(TBL_NM)='dcmainADD' THEN 'dcmainADD.' ELSE 'dcmain.' END )+RTRIM(FLD_NM) else 'dcitem.'+RTRIM(FLD_NM) end as flds,FLD_NM=RTRIM(FLD_NM),att_file,data_ty,head_nm
Into #tmpFlds 
From Lother Where e_code='dc'and DISP_MIS=1
union all
Select case when att_file=1 then 'dcmain.'+RTRIM(FLD_NM) else 'dcitem.'+RTRIM(FLD_NM) end as flds,FLD_NM=RTRIM(FLD_NM),att_file,data_ty='',head_nm  From DcMast
Where Entry_ty='dc'	and DISP_MIS=1
--Added by Divyang P on 17032020 for Bug-33349  Start
union all
Select case when att_file=1 then 'dcmain.'+RTRIM(pert_name) else 'dcitem.'+RTRIM(pert_name) end as flds,FLD_NM=RTRIM(pert_name),att_file,data_ty='',head_nm= (rtrim(head_nm)+'PER')  From DcMast
Where Entry_ty='dc' and DISP_MIS=1
...
SET @SQLCOMMAND = N''+@QueryString+''+N''+@Tot_flds+''+' into '+'##main11'+' FROM dcmain' 
 ...
 SET @SQLCOMMAND =	@SQLCOMMAND+'and (dcmain.inv_sr BETWEEN '+CHAR(39)+cast(@FSeries as varchar)+CHAR(39)+'  AND '+CHAR(39)+cast(@TSeries as varchar )+CHAR(39)+') and (dcmain.Dept BETWEEN '+CHAR(39)+@FDept+CHAR(39)+' AND '+CHAR(39)+@TDept+CHAR(39)+')'
 SET @SQLCOMMAND =	@SQLCOMMAND+'and (dcmain.cate BETWEEN '+CHAR(39)+cast(@FCategory as varchar)+CHAR(39)+'  AND '+CHAR(39)+cast(@TCategory as varchar )+CHAR(39)+') and (DCitem.ware_nm BETWEEN '+CHAR(39)+@FWarehouse+CHAR(39)+' AND '+CHAR(39)+@TWarehouse+CHAR(39)+')'
 SET @SQLCOMMAND =	@SQLCOMMAND+' ORDER BY dcmain.INV_SR,dcmain.INV_NO'
 print @sqlcommand
execute sp_executesql @SQLCOMMAND

--SET @SQLCOMMAND ='select Date,inv_no,Party_nm,GSTIN,State,st_type,item,HSNCODE,qty,stkunit,rate,Total,ITEMTAX,u_asseamt,CGST_PER,CGST_AMT,SGST_PER,SGST_AMT,IGST_PER,IGST_AMT,TotalGST,EWBN,EWBDT,EWBDIST,IT_GROAMT,ITEMNTX,net_amt,ITADDLESS,transname,u_vehno,U_LRDT,u_lrno,EWBVTD,LINERULE,CCESSRATE=case when CCESSRATE=''NO-CESS'' then '''' else CCESSRATE end,COMPCESS from ##main11'	--Commented by Divyang P on 17032020 for Bug-33349

--Added by Divyang P on 17032020 for Bug-33349  Start
SET @SQLCOMMAND=''
SET @SQLCOMMAND ='select a.Date,a.inv_no,a.Party_nm,a.GSTIN,a.State,a.st_type,a.item,a.HSNCODE,a.qty,a.stkunit,a.rate,a.Total,a.ITEMTAX,a.u_asseamt,a.CGST_PER,a.CGST_AMT,a.SGST_PER,a.SGST_AMT,a.IGST_PER,a.IGST_AMT,a.TotalGST,a.EWBN,a.EWBDT,a.EWBDIST,a.IT_GROAMT,a.ITEMNTX,a.net_amt,a.ITADDLESS,a.transname,a.u_vehno,a.U_LRDT,a.u_lrno,a.EWBVTD,a.LINERULE,CCESSRATE=case when a.CCESSRATE=''NO-CESS'' then '''' else a.CCESSRATE end,a.COMPCESS '
SET @SQLCOMMAND = @SQLCOMMAND + N''+@Tot_flds+''+ ' from ##main11 a inner join dcmain on (dcmain.tran_cd=a.tran_cd) inner join dcitem on (dcitem.tran_cd=dcmain.tran_cd) '
--Added by Divyang P on 17032020 for Bug-33349  END
