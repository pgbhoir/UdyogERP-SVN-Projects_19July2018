...
Into #tmpFlds 
From Lother Where e_code='LR' and DISP_MIS=1
union all
...
Where Entry_ty='LR' and DISP_MIS=1
union all
...
SET @SQLCOMMAND = @SQLCOMMAND + N''+@Tot_flds+''+ '  from ##main11 a inner join irmain on (irmain.tran_cd=a.tran_cd) inner join iritem on (iritem.tran_cd=irmain.tran_cd) '	--Added by Divyang P on 18032020 for Bug-33349  END
execute sp_executesql @SQLCOMMAND
drop table ##main11
