If Exists(Select [name] From SysObjects Where xType='P' and [name]='Usp_Rep_MIS_MREV3')
Begin
	Drop Procedure Usp_Rep_MIS_MREV3
End
GO
...
Create PROCEDURE [dbo].[Usp_Rep_MIS_MREV3] 
...
Into #tmpFlds 
From Lother Where e_code='R1' and DISP_MIS=1
union all
...
Where Entry_ty='R1' and DISP_MIS=1
union all
...
SET @SQLCOMMAND=''
--SET @SQLCOMMAND = N''+@QueryString+''+N''+' into '+'##main11'+' FROM irmain'		--Commented by Divyang P on 18032020 for Bug-33349
SET @SQLCOMMAND = N''+@QueryString+''+N''+@Tot_flds+''+' into '+'##main11'+' FROM irmain'	--Added by Divyang P on 18032020 for Bug-33349
 ...
 SET @SQLCOMMAND =	@SQLCOMMAND+'WHERE irmain.entry_ty=''R1'' and (irmain.date BETWEEN '+CHAR(39)+cast(@FrmDate as varchar)+CHAR(39)+'  AND '+CHAR(39)+cast(@Todate as varchar )+CHAR(39)+') and (irmain.Party_nm BETWEEN '+CHAR(39)+@FParty+CHAR(39)+' AND '+CHAR(39)+@TParty+CHAR(39)+')'
...
SET @SQLCOMMAND ='select a.Date,a.inv_no,a.Party_nm,a.GSTIN,a.State,a.st_type,a.item,a.HSNCODE,a.qty,a.stkunit,a.rate,a.Total,a.u_asseamt,a.CGST_PER,a.CGST_AMT,a.SGST_PER,a.SGST_AMT,a.IGST_PER,a.IGST_AMT,a.TotalGST,a.EWBN,a.EWBDT,a.EWBDIST,a.IT_GROAMT,a.net_amt,a.transname,a.u_vehno,a.U_LRDT,a.u_lrno,a.EWBVTD,CCESSRATE=case when a.CCESSRATE=''NO-CESS'' then '''' else a.CCESSRATE end,a.COMPCESS
					,a.prodtype,a.MINVNO,a.MDATE,a.qty_used,BALQTY=a.MQTY -(a.QTY_USED+a.WASTAGE),a.rmqty	'
...
--exec sp_executesql N'execute Usp_Rep_MIS_MREV3 @FDate,@TDate,@FParty,@TParty',N'@FParty nvarchar(4000),@TParty nvarchar(10),@FDate nvarchar(10),@TDate nvarchar(10)',@FParty=N'',@TParty=N'WORK ORDER',@FDate=N'04/01/2019',@TDate=N'03/31/2020'
--go
