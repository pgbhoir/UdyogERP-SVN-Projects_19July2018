If Exists(Select [name] From SysObjects Where xType='P' and [name]='USP_REP_ImportDetail_MIS')
Begin
	Drop Procedure USP_REP_ImportDetail_MIS
End
GO
...
Create PROCEDURE [dbo].[USP_REP_ImportDetail_MIS]
...
SET @QueryString ='SELECT ptmain.entry_ty,ptitem.item as prod_nm,ptitem.party_nm as suppl_nm,ptmain.pinvno as bill_no
,ptmain.pinvdt as bill_dt,ptitem.qty*ptitem.rate as pur_cost,ptitem.gro_amt as cost_price,cogam.ac_name as ledger_nm,COGSALL.allocamt as led_amt'
SET @QueryString =@QueryString+' ,case when COGSALL.entry_ty in(''PT'',''P1'') then pm.pinvno else epmain.pinvno end as transporter_bill_no'
SET @QueryString =@QueryString+' ,case when COGSALL.entry_ty in(''PT'',''P1'') then pm.pinvdt else epmain.pinvdt end as transporter_bill_dt '
set @Tot_flds =''
...
SET @SQLCOMMAND= @SQLCOMMAND+' INNER JOIN PTITEM  ON PTMAIN.TRAN_CD=PTITEM.TRAN_CD LEFT JOIN IT_MAST ON IT_MAST.IT_CODE=PTITEM.IT_CODE INNER JOIN AC_MAST ON PTMAIN.AC_ID=AC_MAST.AC_ID '
...
SET @SQLCOMMAND =	@SQLCOMMAND+' left join epmain on epmain.entry_ty=cogsall.entry_ty and epmain.tran_cd=cogsall.tran_cd'
SET @SQLCOMMAND =	@SQLCOMMAND+' left join ptmain pm on pm.entry_ty=cogsall.entry_ty and pm.tran_cd=cogsall.tran_cd'
SET @SQLCOMMAND =	@SQLCOMMAND+' inner join AC_MAST cogam on cogam.Ac_id=COGSALL.ac_id'
...
execute sp_executesql @sqlcommand
--set dateformat dmy
--execute USP_REP_Import_MIS '01/06/2020','30/06/2020','','pt test','',''
